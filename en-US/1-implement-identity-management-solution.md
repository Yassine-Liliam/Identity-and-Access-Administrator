# Implement an identity management solution using Entra ID
...
- **Implement initial configuration of Entra ID** (11 units)
...
# Implement initial configuration of Entra ID
...
In this module, you learn how to configure and manage an Entra tenant. You explore Entra roles, custom domains, and company branding options. In addition, you learn how to configure delegation by using administrative units and configure several tenant-wide settings within Entra ID.
...
- Configure and manage Entra roles.
...
Prior usage experience with the Azure portal user interface or the Entra admin center.


## Configure company brand

You can use your organization's logo and custom color schemes to provide a consistent experience on your sign-in pages. Your sign-in pages appear when users sign in to your organization's web-based apps, such as Microsoft 365, which uses Entra ID as your identity provider. Adding custom branding requires you to have either **Entra ID premium P1, P2, or Office 365 (for Office 365 apps)** license.

To set the company branding, open up the Entra ID page in the Azure portal. Then launch **Company branding** from the Manage menu. A premium license is required for the menu option to be present.
...
Entra ID is Microsoft’s cloud-based identity and access management service, which helps your employee's sign-in and access resources in:
...
Entra ID is intended for:

- **IT admins** - As an IT admin, you can use Entra ID to control access to your apps and your app resources, based on your business requirements. For example, you can use Entra ID to require multifactor authentication when accessing important organizational resources. Additionally, you can use Entra ID to automate user provisioning between your existing Windows Server AD and your cloud apps, including Microsoft 365. Finally, Entra ID gives you powerful tools to automatically help protect user identities and credentials and to meet your access governance requirements.
- **App developers** - Use Entra ID as a standards-based way to add single sign-on (SSO) to your app, so it works with a user's existing credentials. Entra ID also provides APIs that can help you build personalized app experiences using existing organizational data.
- **Microsoft 365, Office 365, Azure, or Dynamics CRM Online subscribers** - As a subscriber, you're already using Entra ID. Each Microsoft 365, Office 365, Azure, and Dynamics CRM Online tenant is automatically an Entra tenant. You can immediately start to manage access to your integrated cloud apps.

In Entra ID, if one of your users needs permission to manage Entra resources, you must assign them to a role that provides the permissions they need.

If you're new to Azure, you might find it a little challenging to understand all the different roles in Azure. The following section helps explain the following roles and provides additional information on Azure roles and Entra roles:
...
Entra roles are used to manage Entra resources in a directory. Actions such as create or edit users are the most common. However, the need to assign administrative roles to others, reset user passwords, manage user licenses, and manage domains are common. The following table describes a few of the more important Entra roles.

| **Entra role** | **Permissions** | **Notes** |
|---|---|---|
| Global Administrator | Manage access to all administrative features in Entra ID, and services that federate to Entra ID | The person who signs up for the Entra tenant becomes the first Global Administrator. |
...
In the Azure portal, you can see the list of Entra roles on the **Roles and administrators** screen.

### Differences between Azure roles and Entra roles

At a high level, Azure roles control permissions to manage Azure resources, while Entra roles control permissions to manage Entra resources. The following table compares some of the differences.

| **Azure roles** | **Entra roles** |
|---|---|
| Manage access to Azure resources | Manage access to Entra resources |
...
#### Do Azure roles and Entra roles overlap?

By default, Azure roles and Entra roles don't span Azure and Entra ID. However, a Global Administrator can elevate their access with the **Access management for Azure resources** switch in the Azure portal. That grants them the User Access Administrator role (an Azure role) on all subscriptions in that tenant. The User Access Administrator role enables the user to grant other users access to Azure resources. This switch can be helpful to regain access to a subscription.

Several Entra roles span Entra ID and Microsoft 365, such as the Global and User Administrator roles. For example, if you're assigned the Global Administrator role, you have administrator capabilities in Entra ID and Microsoft 365, such as making changes to Microsoft Exchange and Microsoft SharePoint. However, by default, the Global Administrator doesn't have access to Azure resources.
...
![Diagram of relationship of Azure roles to Entra roles. Azure roles accessed in Azure tenant.  Entra roles also accessed from Entra ID and Microsoft 365.](https://learn.microsoft.com../../wwl-sci/implement-initial-configuration-of-azure-active-directory/media/azure-office-roles.png)

### Assign roles

There are multiple ways to assign roles within Entra ID. You need to pick the one that best meets your needs. The user interface might be slightly different for each method, however the configuration options are similar. Methods for assigning roles include:

- Assign a role to a user or group
  - **Entra ID** - **Roles and administration** - **Select a role** - **+ Add Assignment**

- Assign a user or group to a role
  - **Entra ID** - Open **Users** (or Groups) - Select an **User** (or group) - **Assigned roles** - **+ Add Assignment**
...
A common way to assign Entra roles to a user is on the Assigned roles page for a user. You can also configure the user eligibility to be elevated just-in-time into a role using **Privileged Identity Management (PIM)**.

Note

If you have an Entra ID Premium P2 license plan and already use PIM, all role management tasks are performed in the Privileged Identity Management experience. This feature is currently limited to assigning only one role at a time. You can't currently select multiple roles and assign them to a user all at once.

### Create and assign a custom role in Entra ID

This section describes how to create new custom roles in Entra ID. For the basics of custom roles, see the [custom roles overview](https://learn.microsoft.com/en-us/azure/active-directory/roles/custom-overview). The role can be assigned either at the directory-level scope or an app registration resource scope only.

Custom roles can be created in the [Roles and administrators](https://portal.azure.com/) tab on the Entra ID overview page.

1. Select **Entra ID** - **Roles and administrators** - **New custom role**.
...
5. You need to an Entra ID P2 license to complete some of the exercises. In the organization you created, search for and then select Entra ID.
6. Select **Licenses** in the menu.
7. In the right **Quick tasks** menu of the Licenses - Overview page, select **Get a free trial**
8. Under Getting started with Entra ID, select **Get a free trial for Entra ID Premium**.
9. In the Activate pane, under Entra ID **PREMIUM P2**, select **Free trial** and then select **Activate**.
10. In the navigation menu on the left, select **Overview**.
11. Refresh the browser until you see Entra ID Premium P2 under the organization name. It might take a couple of minutes.
...
Using Entra ID, you can designate limited administrators to manage identity tasks in less-privileged roles. Administrators can be assigned for such purposes as adding or changing users, assigning administrative roles, resetting user passwords, managing user licenses, and managing domain names.

1. In Entra ID, All users screen, select **Adele Vance**.
...
1. In Entra ID, select **Users - All User**, and then select the user getting the role assignment removed. For example, *Adele Vance*.
...
Administrative units are Entra ID resources that can be containers for other Entra resources. An administrative unit can contain only users, groups, and devices.
...
In Entra ID, using a single tenant if you assign a user any administrator role, they're now an admin over every user in the tenant. Always think about the security principle of least privilege, it's always the best way to grant administrative responsibilities. Administrative units are containers created to solve for this challenge in Entra ID. If you want a User Administrator to be able to manage only a specific set of users and group. Say to only manage users in the Research Department of a hospital. You could set up an administrative unit. In that administrative unit, you add the users and groups for the research team. Then you assign one user, call them Admin-for-research, to the User Administrator role within the unit. Admin-for-research would be able to manage the users in the administrative unit but not across the entire tenant, which helps to achieve the principle of least privilege.
...
You can use administrative units to logically group Entra resources. An organization whose IT department is scattered globally might create administrative units that define relevant geographical boundaries. In another scenario, where a global organization has suborganizations that are semi-autonomous in their operations, administrative units could represent the suborganizations.
...
With organizational growth comes complexity. One common response is to reduce some of the workload of access management with Entra admin roles. You can assign the least possible privilege to users to access their apps and perform their tasks. There are many reasons for an organization move toward a more decentralized administration.

In Entra ID, you can delegate Application creation and management permissions in the following ways:

- Restricting who can create applications and manage the applications they create. By default in Entra ID, all users can register application registrations and manage all aspects of applications they create. You can restrict to only allow selected people that permission.
- Assigning one or more owners to an application. A simple way to grant someone the ability to manage all aspects of Entra ID configuration for a specific application.
- Assigning a built-in administrative role that grants access to manage configuration in Entra ID for all applications. The recommended way to grant IT experts access to manage broad application configuration permissions without granting access to manage other parts of Entra ID not related to application configuration.
...
- The **Application Administrator** role, which can manage all applications in the directory: registrations, single sign-on settings, user and group assignments, licensing, Application Proxy settings, and consent. It doesn't grant the ability to manage Conditional Access.
...
Entra ID provides an extensive guide to planning and executing a security plan on your Entra admin roles, [Securing privileged access for hybrid and cloud deployments](https://learn.microsoft.com/en-us/azure/active-directory/roles/security-planning).
...
If possible, you only want to grant the minimum permissions a user needs. So be sure to know what all permissions are granted when you assign a role. You can see the list of permissions in the description of each role. To open, launch Entra ID, then open the **Roles and administrators** screen. Next select a role, and open its description page from the ellipsis (...) menu. Depending on the role you chose, you'll see a large or small number of permissions. Two sets of permissions:
...
A domain name is part of the identifier for many Entra ID resources: a user's name or email address, a group's address, and sometimes an application's app ID URI. A resource in Entra ID can include a domain name that's owned by the organization that contains the resource. Only a Global Administrator can manage domains in Entra ID.

### Set the primary domain name for your Entra organization
...
### Add custom domain names to your Entra organization
...
If you want to add a subdomain name such as ‘europe.contoso.com’ to your organization, you should first add and verify the root domain, such as contoso.com. The subdomain is automatically verified by Entra ID. To see the verified subdomain you added, refresh the domain list in the browser.

If you've already added a contoso.com domain to one Entra organization, you can also verify the subdomain europe.contoso.com in a different Entra organization. When adding the subdomain, you're prompted to add a TXT record in the DNS hosting provider.

### What to do if you change the DNS registrar for your custom domain name

If you change the DNS registrars, there are no additional configuration tasks in Entra ID. You can continue using the domain name with Entra ID without interruption. If you use your custom domain name with Microsoft 365, Intune, or other services that rely on custom domain names in Entra ID, see the documentation for those services.

### Delete a custom domain name

You can delete a custom domain name from your Entra ID if your organization no longer uses it, or if you need it in another Entra ID.
...
- Any application in your Entra ID has an app ID URI that includes the domain name.

You must change or delete any such resource in your Entra organization before you can delete the custom domain name.

#### ForceDelete option

**ForceDelete** can be used to remove a domain name in the Entra admin center or using Microsoft Graph API. These options use an asynchronous operation and update all references from the custom domain name like “user@contoso.com” to the initial default domain name such as “user@contoso.onmicrosoft.com.”

To call **ForceDelete** in the Azure portal, the domain name must have fewer than 1000 references. Any reference where Exchange is the provisioning service must first be updated or removed in the Exchange Admin Center. Exchange Mail-Enabled Security Groups and distributed lists are included. Also, the **ForceDelete** operation won't succeed if either of the following is true:
...
In Entra ID, all users are granted a set of default permissions. A user’s access consists of the type of user, their role assignments, and their ownership of individual objects. The default user permissions can be changed only in user settings in Entra ID.
...
| Restrict access to Entra administration portal | Setting this option to No lets non-administrators use the Entra administration portal to read and manage Entra resources. Yes restricts all non-administrators from accessing any Entra data in the administration portal. |
|   | This setting doesn't restrict access to Entra data using PowerShell or other clients such as Visual Studio. When set to Yes, to grant a specific non-admin user the ability to use the Entra administration portal assign any administrative role such as the Directory Readers role. |
...
- **Guest user access** - Guest rights range from acting almost as a full user, down to seeing only their own content.
...
Set the basic values that define the look at feel of your tenant within Entra ID.
...
1. In the **Entra ID** screen, in the Manage section, select **Properties**.
...
Just as the Country / region is found in the Entra ID Properties dialog, so is the location information.

1. In the **Properties** screen, under **Tenant properties**, locate **Location** and review the information.

#### Find the tenant ID

Azure subscriptions have a trust relationship with Entra ID. Entra ID is trusted to authenticate users, services, and devices for the subscription. Each subscription has a tenant ID associated with it, and there are a few ways you can find the tenant ID for your subscription.
...
You add your organization's privacy information in the **Properties** area of Entra ID. To access the Properties area and add your privacy information:
...
- Information about which roles manage Azure resources and which roles manage Entra resources is available at [Classic subscription administrator roles, Azure roles, and Entra roles](https://learn.microsoft.com/en-us/azure/role-based-access-control/rbac-and-directory-admin-roles).
...
- The following step-by-step guides provide information on how you can use Conditional Access to configure equivalent policies to those policies enabled by security defaults:
  ...
  - [Require MFA registration](https://learn.microsoft.com/en-us/azure/active-directory/identity-protection/howto-identity-protection-configure-mfa-policy) - Requires Entra Identity Protection part of Entra ID Premium P2.
...
Every user who needs access to resources needs a user account in Entra ID. A user account contains all the information needed to authenticate the user during the sign-on process. Once authenticated, Entra ID builds an access token to authorize the user and determine what resources they can access and what they can do with those resources.

You use the **Entra admin center** to work with user objects. Keep in mind that you can only work with a single directory at a time. You can use the **Directory + Subscription** panel to switch directories. The admin center also has a **Switch directory** button in the toolbar, which makes it easy to switch to another available directory.

### View users

To view the Entra users, select the **Users** entry under **Identity**, then open the **All Users** view. Take a minute to access the admin center and view your users. Notice the **User Type** column to see members and guests, as the following figure depicts.

Typically, Entra ID defines users in three ways:

- **Cloud identities** - These users exist only in Entra ID. Examples are administrator accounts and users that you manage yourself. Their source is **Entra ID** or **External Entra directory** if the user is defined in another Entra instance but needs access to subscription resources controlled by this directory. When these accounts are removed from the primary directory, they're deleted.
- **Directory-synchronized identities** - These users exist in an on-premises Active Directory. A synchronization activity brings these users into Entra ID. **Entra Cloud Sync** is the recommended synchronization tool for most organizations—it uses a lightweight cloud-managed agent and supports multiple disconnected forests. **Entra Connect Sync** remains available for complex scenarios such as device synchronization or groups with more than 50,000 members. Their source is **Windows Server AD**.
...
**Exercise environment needs** - this lab assumes you have a basic Entra tenant with at least User Administrator rights to complete it. You can get a free trial subscription for at [Try Azure for Free](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).

### Create a new user in Entra ID

You can skip creating this user if you created the same user in the earlier module.

1. Browse to the Identity menu in the [Entra admin center](https://entra.microsoft.com/).
...
1. Browse to the Entra admin center screen.
...
### Restore or remove a recently deleted user with Entra ID
...
You can view your restorable users, restore a deleted user, or permanently delete a user using Entra ID user interface.
...
**Exercise environment needs** - this lab assumes you have a basic Entra tenant with at least User Administrator rights to complete it. You can get a free trial subscription at [Try Microsoft Azure for free](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).
...
2. Review the list of deleted users and select the user you deleted.  Important By default, deleted user accounts are permanently removed from Entra ID automatically after 30 days.
...
An Entra group helps organize users, which makes it easier to manage permissions. Groups let the resource owner (or Entra directory owner) assign one set of access permissions to every member at once, instead of granting the rights one by one. Groups let you define a security boundary and then add and remove specific users to grant or deny access with a minimum amount of effort. Even better, Entra ID supports the ability to define membership based on rules - such as what department a user works in, or the job title they have.

Entra ID allows you to define two different types of groups.

- **Security groups** - the most common type of groups and are used to manage access to shared resources. Members of a security group can include users, devices, and service principals. For example, you can create a security group for a specific security policy. By doing it this way, you can give a set of permissions to all the members at once, instead of having to add permissions to each member individually. This option requires an Entra administrator.
...
You can view all groups through the **Groups** item under **Identity** in the Entra admin center. A new Entra ID deployment has no groups defined.
...
With dynamic membership, Entra ID automatically adds or removes users or devices from a group based on rules you define. When a member's attributes change—for example, the user moves to a different department—the tenant reevaluates all dynamic membership rules, then adds or removes the user from groups accordingly.

Dynamic membership requires a **Entra ID P1** license (or Intune for Education for device-based rules).
...
## Exercise - add groups in Entra ID

**Exercise environment needs** - this lab assumes you have a basic Entra tenant with at least User Administrator rights to complete it. You can get a free trial subscription at [Try Microsoft Azure for free](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).

### Create a Microsoft 365 group in Entra ID
...
The goal of Entra registered devices is to provide your users with support for the BYOD or mobile device scenarios. In these scenarios, a user can access your organization’s Entra ID controlled resources using a personal device.

| **Entra registered** | **Description** |
|---|---|
| Definition | Registered to Entra ID without requiring organizational account to sign in to the device |
...
![Diagram of Entra registered devices. Shows a laptop and cell registered.](https://learn.microsoft.com../../wwl-sci/create-configure-manage-identities/media/azure-active-directory-registered-device.png)

You sign in to an Entra registered device with a local account, such as a Microsoft account on Windows 10 or newer. The device also has an Entra account attached, which gives access to organizational resources. Access to resources in the organization can be further limited based on that Entra account and Conditional Access policies applied to the device identity.

Administrators can secure and further control these Entra registered devices using Mobile Device Management (MDM) tools like Microsoft Intune. MDM provides a means to enforce organization-required configurations like requiring storage to be encrypted, password complexity, and security software kept updated.

Entra ID registration can be accomplished when accessing a work application for the first time or manually using the Windows 10 or Windows 11 Settings menu.

#### Scenarios for registered devices

A user in your organization wants to access tools for email, reporting time-off, and benefits enrollment from their home PC. Your organization has these tools behind a Conditional Access policy that requires access from an Intune compliant device. The user adds their organization account and registers their home PC with Entra ID and the required Intune policies are enforced giving the user access to their resources.
...
Entra joined is intended for organizations that want to be cloud-first or cloud-only. Any organization can deploy Entra joined devices no matter the size or industry. Entra joined enables access to both cloud and on-premises apps and resources.

| **Entra joined** | **Description** |
|---|---|
| Definition | Joined only to Entra ID requiring organizational account to sign in to the device |
...
Entra joined devices are signed in to using an organizational Entra account. Access to resources in the organization can be further limited based on that Entra account and Conditional Access policies applied to the device identity.

Administrators can secure and further control Entra joined devices using Mobile Device Management (MDM) tools like Microsoft Intune or in co-management scenarios using Microsoft Endpoint Configuration Manager. These tools provide a means to enforce organization-required configurations like requiring storage to be encrypted, password complexity, software installations, and software updates. Administrators can make organization applications available to Entra joined devices using Configuration Manager.

Entra joined can be accomplished using self-service options like the Out of Box Experience (OOBE), bulk enrollment, or Windows Autopilot.

Entra joined devices can still maintain single sign-on access to on-premises resources when they are on the organization's network. Entra joined devices authenticate to on-premises servers like for file, print, and other applications.

#### Scenarios for joined devices

Although Entra joined is primarily intended for organizations that don't have an on-premises Windows Server Active Directory infrastructure, you can certainly use it in scenarios where:
...
- You want to manage a group of users in Entra ID instead of in Active Directory. This scenario can apply, for example, to seasonal workers, contractors, or students.
- You want to provide joining capabilities to workers in remote branch offices with limited on-premises infrastructure.

You can configure Entra joined devices for all Windows 10 and Windows 11 devices except for the Home editions.

The goal of Entra joined devices is to simplify:

- Windows deployments of work-owned devices
- Access to organizational apps and resources from any Windows device
- Cloud-based management of work-owned devices
- Users to sign in to their devices with their Entra ID or synced Active Directory work or school accounts.

![Diagram of Entra joined devices connected to the cloud. A laptop registered to your cloud directory.](https://learn.microsoft.com../../wwl-sci/create-configure-manage-identities/media/azure-active-directory-joined-device.png)

Entra Joined can be deployed by using many different methods.
...
If your environment has an on-premises AD footprint and you also want benefit from the capabilities provided by Entra ID, you can implement hybrid Entra joined devices. These devices are devices that are joined to your on-premises Active Directory and registered with your Entra directory.

| **Hybrid Entra joined** | **Description** |
|---|---|
| Definition | Joined to on-premises AD and Entra ID requiring organizational account to sign in to the device |
...
![Diagram of the process flow of Hybrid Entra joined devices. A laptop is registered to an on-premises active directory.](https://learn.microsoft.com../../wwl-sci/create-configure-manage-identities/media/azure-active-directory-hybrid-joined-device.png)

#### Scenarios for hybrid joined

Use Entra hybrid joined devices if:
...
Device writeback is no longer supported and is no longer a recommended approach for hybrid identity scenarios. It's replaced by **Cloud Kerberos Trust**. This lets Entra joined and hybrid joined devices authenticate to on-premises resources without writing device objects back to on-premises Active Directory.

For organizations planning new hybrid deployments, use Cloud Kerberos Trust to enable on-premises SSO and Windows Hello for Business in hybrid environments. See [Configure Entra Kerberos for on-premises single sign-on](https://learn.microsoft.com/en-us/entra/identity/authentication/howto-authentication-passwordless-security-key-on-premises) for guidance.


## Manage licenses

Microsoft paid cloud services, such as Microsoft 365, Enterprise Mobility + Security, Dynamics 365, and other similar products, require licenses. These licenses are assigned to each user who needs access to these services. To manage licenses, administrators use the [Microsoft 365 admin center](https://admin.microsoft.com/) or PowerShell and Microsoft Graph API. Entra ID is the underlying infrastructure that supports identity management for all Microsoft cloud services. Entra ID stores information about license assignment states for users.

Without group-based licensing, assigning licenses at the individual user level makes large-scale management difficult. For example, an administrator often has to write a complex PowerShell script to add or remove user licenses when people join or leave the organization or a department. This script makes individual calls to the cloud service.

To address those challenges, Entra ID now includes group-based licensing. You can assign one or more product licenses to a group. Entra ID ensures that the licenses are assigned to all members of the group. Any new members who join the group are assigned the appropriate licenses. When they leave the group, those licenses are removed. This licensing management eliminates the need for automating license management via PowerShell to reflect changes in the organization and departmental structure on a per-user basis.

### License requirements

You must have one of the following licenses to use group-based licensing:

- Paid or trial subscription for Entra ID Premium P1 and greater
...
- Licenses can be assigned to any security group in Entra ID. Security groups can be synced from on-premises by using Entra Cloud Sync (recommended) or Entra Connect Sync. You can also create security groups directly in Entra ID (also called cloud-only groups), or automatically via the Entra dynamic group feature.
...
- Entra ID automatically manages license modifications that result from group membership changes. Typically, license modifications are effective within minutes of a membership change.
...
1. Open [https://entra.microsoft.com](https://entra.microsoft.com) to get to the Entra admin center.
...
7. Pick the group you were looking at earlier in Entra. Then select the **Assign** button at the bottom of the page.
8. On the group’s Licenses page, review the change. You should be able to see the change in both the Entra admin center and the Microsoft 365 admin center.

### Identify and resolve license assignment problems for a group in Entra ID

Group-based licensing in Entra ID introduces the concept of users in a licensing error state. In this section, we explain the reasons why users might end up in this state.
...
To see how many licenses are available, go to **Entra - Identity - Billing** , then **Licenses**, then **All products**.
...
The decision about how to resolve conflicting product licenses always belongs to the administrator. Entra ID doesn't automatically resolve license conflicts.

**PowerShell**: PowerShell cmdlets report this error as *MutuallyExclusiveViolation*.

### Other products depend on this license

**Problem**: One of the products specified in the group contains a service plan that must be enabled for another service plan, in another product, to function. This error occurs when Entra ID attempts to remove the underlying service plan. For example, this can happen when you remove the user from the group.

To solve this problem, either keep the required plan assigned to those users by another method, or disable the dependent services for them. After doing that, you can properly remove the group license from those users.
...
When Entra ID attempts to assign a group license to a user whose usage location isn't supported, it fails and records an error on the user.

To solve this problem, remove users from unsupported locations from the licensed group. Alternatively, if the current usage location values don't match where the users actually are, change them so licenses are assigned correctly next time (if the new location is supported). You can specify the usage location under the user's **Properties** tab in the [Entra admin center](https://entra.microsoft.com).

**PowerShell**: PowerShell cmdlets report this error as *ProhibitedInUsageLocationViolation*.

Note

When Entra ID assigns group licenses, any users without a specified usage location inherit the location of the directory. We recommend that administrators set the correct usage location values on users before using group-based licensing to comply with local laws and regulations.
...
### Entra Mail and ProxyAddresses attribute change

**Problem**: While updating license assignment on a user or a group, you might see that the Entra Mail and ProxyAddresses attribute of some users are changed.
...
**Problem**: User has LicenseAssignmentAttributeConcurrencyException for license assignment in audit logs. When group-based licensing tries to process concurrent license assignment of the same license to a user, this exception is recorded on the user. This typically happens when a user is a member of more than one group with same assigned license. Entra ID retries processing the user license and will resolve the issue. There's no action required from the customer to fix this issue.
...
Some Microsoft Online products you might own are *add-ons*. Add-ons require a prerequisite service plan to be enabled for a user or a group before they can be assigned a license. Group-based licensing requires the prerequisite and the add-on service plans in the same group, so every user added to the group receives the fully working product. Let's consider the following example:
...
You can create multiple groups for each prerequisite service plan. For example, if your users have both Office 365 Enterprise E1 and Office 365 Enterprise E3, create two groups to license Microsoft Workplace Analytics: one with E1 as the prerequisite, the other with E3. This lets you distribute the add-on to E1 and E3 users without consuming more licenses.
...
For example, if you free up licenses by removing direct assignments from users, trigger processing again for the groups that previously failed to license all their members. To reprocess a group, go to the group pane, open **Licenses**, and then select the **Reprocess** button on the toolbar.
...
1. Using the Azure portal, assign the Office 365 E3 license to the **All users** group in Entra ID.
...
### Change license assignments for a user or group in Entra ID

This section describes how to move users and groups between service license plans in Entra ID. The goal is to ensure that there's no loss of service or data during the license change. Users should switch between services seamlessly. This section describes moving a user or group from Office 365 E1 to Office 365 E3, but the steps apply to all license plans. When you update license assignments for a user or group, the removals and the new assignments happen at the same time. Users therefore keep access to their services during the change and see no license conflicts between plans.
...
- If you manage groups on-premises and sync them into Entra ID via Entra Connect, then you add or remove users by using your on-premises system. It can take some time for the changes to sync with Entra ID to be picked up by group licensing.
- If you're using Entra dynamic group memberships, you add or remove users by changing their attributes, but the update process for license assignments remains the same.
...
7. When complete, verify the license is listed on Dominique Koch's profile in the Entra admin center under **Identity** > **Users** > select the user > **Licenses**.
...
Custom security attributes in Entra ID are business-specific attributes (key-value pairs) that you can define and assign to Entra objects. These attributes can be used to store information, categorize objects, or enforce fine-grained access control over specific Azure resources.
...
- Add custom security attributes to Entra users and enterprise applications (service principals).
- Manage Entra objects using custom security attributes with queries and filters.
- Provide attribute governance so attributes determine who can get access.

Custom security attributes are **not** supported in Entra Domain Services, SAML token claims, or JSON Web Token (JWT) claims.
...
![Diagram of the process flow for auto user provisioning in Entra ID. Auto provision users and groups.](https://learn.microsoft.com../../wwl-sci/create-configure-manage-identities/media/automatic-user-provisioning.png)
...
- **Entra Provisioning Service** - Uses the SCIM 2.0 protocol for automatic provisioning. The service connects to the SCIM endpoint for the application, and uses the SCIM user object schema and REST APIs to automate provisioning and deprovisioning of users and groups.
- **Entra ID** - User repository used to manage the lifecycle of identities and their entitlements.
- **Target system** - Application or system that has SCIM endpoint and works with the Entra provisioning to enable automatic provisioning of users and groups.

#### Why use SCIM?

System for Cross-Domain Identity Management (SCIM) is an open standard protocol for automating the exchange of user identity information between identity domains and IT systems. SCIM ensures that employees added to the Human Capital Management (HCM) system automatically have accounts created in Entra ID or Windows Server Active Directory. User attributes and profiles are synchronized between the two systems, updating, or removing users based on the user status or role change.

The key is keeping your identity systems up to date. If a user can be automatically deprovisioned from Entra ID as soon as they're removed from your HR systems, you have less worry about a possible breach.

#### API-driven inbound provisioning

Not all HR systems expose a SCIM endpoint. For these scenarios, Entra ID supports **API-driven inbound provisioning**, which reached general availability in March 2024. The source system doesn't have to push data via SCIM: any automation tool or script can retrieve workforce data from any system of record and send it to the Entra provisioning API. Supported authoritative sources include Workday, SAP SuccessFactors, and any custom HR system integrated via the API. This approach gives organizations flexibility to automate identity lifecycle management regardless of their HR platform's native integration capabilities.
...
- [Create a basic group and add members using Entra ID](https://learn.microsoft.com/en-us/entra/fundamentals/how-to-manage-groups)
- [Create or update a dynamic membership group in Entra ID](https://learn.microsoft.com/en-us/entra/identity/users/groups-create-rule)
...
- [Plan Entra device deployment](https://learn.microsoft.com/en-us/entra/identity/devices/plan-device-deployment)
...
Being able to invite external users to use your Azure resources is a great benefit, but it needs to be done in a secure way. This module shows you how to enable secure B2B collaboration with users outside your organization, including how to manage external collaboration settings in Entra ID and how to invite users individually or in bulk. You will also learn about managing external user accounts and configuring identity providers.

### Learning objectives

In this module, you:

- Manage external collaboration settings in Entra ID.
- Invite external users (individually or in bulk).
- Manage external user accounts in Entra ID.
- Configure identity providers (social and SAML/WS-fed).
- Explore Entra Verified ID.
...
![Diagram of allowing external users to join your Entra tenant as a guest user.](https://learn.microsoft.com../../wwl-sci/implement-manage-external-identities/media/guest-user-diagram.png)

#### Define guest users

Entra B2B collaboration is a feature within Entra External Identities, part of Entra that lets you invite guest users to collaborate with your organization. With B2B collaboration, you can securely share your company's applications and services with external users, while maintaining control over your own corporate data. Work safely and securely with external partners, large or small, even if they don't have Entra ID or an IT department.

#### How guest users join your Entra tenant
...
Developers can use Entra ID business-to-business APIs to customize the invitation process or write applications like self-service sign-up portals.

#### B2B collaboration

B2B collaboration is a capability of Entra External Identities that lets you collaborate with users and partners outside of your organization. With B2B collaboration, an external user is invited to sign in to your Entra organization using their own credentials. This B2B collaboration user can then access the apps and resources you want to share with them. A user object is created for the B2B collaboration user in the same directory as your employees. B2B collaboration user objects have limited privileges in your directory by default, and they can be managed like employees, added to groups, and so on.


## Manage external collaboration

Entra External Identities is a feature that makes it possible for you to allow people outside your organization to access your apps and resources. Your partners, distributors, suppliers, vendors, and other guest users can "bring their own identities." They sign in with their own credentials, whether those come from a corporate or government-issued digital identity or from an unmanaged social identity like Google or Facebook. The external user’s identity provider manages their identity, and you manage access to your apps with Entra ID to keep your resources protected.

### Invitation redemption flow

![Diagram of the redemption of an external invitation to join Entra tenant as a guest.](https://learn.microsoft.com../../wwl-sci/implement-manage-external-identities/media/business-to-business-invitation-redemption.png)

1. Entra ID runs user-based discovery to see whether the user already exists in a managed Entra tenant. (Unmanaged Entra accounts can no longer be used for redemption.) If the user’s User Principal Name (UPN) matches both an existing Entra account and a personal MSA, the user is asked to choose which account to redeem with.
2. If an admin enabled SAML/WS-Fed IdP federation, Entra ID checks the user’s domain suffix against the domain of a configured SAML/WS-Fed identity provider, then redirects the user to that pre-configured provider.
3. If an admin has enabled Google federation, Entra ID checks if the user’s domain suffix is gmail.com, or googlemail.com and redirects the user to Google.
...
6. If no home directory is found and the email one-time passcode feature is enabled for guests, a passcode is sent to the user through the invited email. The user retrieves and enters this passcode in the Entra sign-in page.
7. If no home directory is found and email one-time passcode for guests is disabled, the user is prompted to create a consumer MSA with the invited email. We support creating an MSA with work emails in domains that aren't verified in Entra ID.
8. The user, after authenticating to the right identity provider, is redirected to Entra ID to complete the consent experience.

### External identities scenarios

Entra External Identities focus less on a user's relationship to your organization and more on how the user wants to sign in to your apps and resources. Within this framework, Entra ID supports various scenarios.

B2B collaboration lets you invite external users into your own tenant as "guest" users. You assign them permissions (for authorization), and they keep using their existing credentials (for authentication). Users sign in to the shared resources using a simple invitation and redemption process with their work, school, or other email account. You can also use Entra entitlement management to configure policies that manage access for external users. And now with the availability of self-service sign up user flows, you can allow external users to sign up for applications themselves. The experience can be customized to allow sign up with a work, school, or social identity (such as Google or Facebook). You can also collect information about the user during the sign up process.
...
- Single sign-on (SSO) - SSO to all Entra-connected apps is supported. For example, you can provide access to Microsoft 365 or on-premises apps, and to other SaaS apps such as Salesforce or Workday.
...
This unit describes how to enable Entra B2B collaboration. Then, we explore the ability to designate who can invite guests and determine the permissions that guests have.
...
- Guest users have the same access as members (most inclusive): This option gives guests the same access to Entra resources and directory data as member users.
...
After you add a guest user to the directory, you can send them a direct link to a shared app, or they can select the redemption URL in the invitation email. Make sure your organization's external collaboration settings are configured such that you're allowed to invite guests. By default, all users and admins can invite guests. But your organization's external collaboration policies might be configured to prevent certain types of users or admins from inviting guests.

### How users in your organization can invite guest users to an app

Once a guest user is in the Entra ID directory, an application owner can send them a direct link to the app they want to share. Entra admins can also set up self-service management for gallery or SAML-based apps in their Entra tenant. This way, application owners can manage their own guest users, even if the guest users haven’t been added to the directory yet. When an app is configured for self-service, the application owner uses their Access Panel to invite a guest user to an app, or to add them to a group that has access to it. Self-service app management for gallery and SAML-based apps requires some initial setup by an admin, which can be summarized as follows:
...
### How to bulk invite Entra B2B collaboration users

If you use Entra B2B collaboration to work with external partners, you can invite multiple guest users to your organization at the same time. Specifically, you do the following:

- Use **Bulk invite users** to prepare a comma-separated value (.csv) file with the user information and invitation preferences
- Upload the .csv file to Entra ID
- Verify the users were added to the directory

#### Understand the CSV template

Download and fill in the bulk upload CSV template to help you successfully invite Entra ID guest users in bulk. The CSV template you download might look like this example:
...
1. Sign in to the [Entra admin center](https://entra.microsoft.com/) as a user who is assigned a limited administrator directory role or the Guest Inviter role.
2. Select **Identity**
...
1. Sign in to the [Entra admin center](https://entra.microsoft.com/) with an account that is a User administrator in the organization.
...
## Demo - manage guest users in Entra ID
...
### Key properties of the Entra B2B collaboration user
...
This property indicates the user’s primary identity provider. A user can have several identity providers. To see them, select the link next to Identities in the user’s profile, or query the identities property with the Microsoft Graph API.

| **Identities property value** | **Sign-in state** |
|---|---|
| External Entra tenant | This user is homed in an external organization and authenticates by using an Entra account that belongs to the other organization. |
| Microsoft account | This user is homed in a Microsoft account and authenticates by using a Microsoft account. |
| {host’s domain} | This user authenticates by using an Entra account that belongs to this organization. |
...
| mail | This user has signed up by using Entra Email one-time passcode (OTP). |
| {issuer URI} | This user is homed in an external organization that doesn't use Entra ID as their identity provider, but instead uses a SAML/WS-Fed-based identity provider. |

#### Can Entra B2B users be added as members instead of guests?

Typically, an Entra B2B user and guest user are synonymous. Therefore, an Entra B2B collaboration user is added as a user with UserType = Guest by default. However, in some cases, the partner organization is a member of a larger organization to which the host organization also belongs. If so, the host organization might want to treat users in the partner organization as members instead of guests. Use the Entra user properties to change a guest into a member.
...
As with any account in Entra ID, you need to review and manage them regularly. Set up procedures to validate all users accounts, especially guest users, regularly. If an account does not need a capability, then remove it. If a user, guest or member, no longer needs a license or access then remove it.

Tools to manage Microsoft 365 guest users:

- Microsoft 365 admin center: `https://admin.microsoft.com`
- Entra admin center: `https://entra.microsoft.com`
- Entra ID within the Azure portal
...
1. Sign in to the [Entra admin center](https://entra.microsoft.com/) with an account that is assigned the User administrator role in the tenant.
2. Select **Identity**.
...
Entra Verified ID safeguards your organization with an identity solution that's seamless and decentralized. The service allows you to issue and verify credentials. For issuers, Entra ID provides a service that they can customize and use to issue their own verifiable credentials. For verifiers, the service provides a free REST API that makes it easy to request and accept verifiable credentials in your apps and services.
...
#### Deploying the Entra Verified ID service

To deploy Entra Verified ID, you will need:

- An Azure tenant with a subscription
- An Entra ID premium license
- Logged in as the global administrator
- A configured Azure Key Vault instance

To set up Entra Verified ID, follow these steps:
...
Direct federation is now called **SAML/WS-Fed identity provider** (IdP) federation. You can set up federation with any organization whose identity provider (IdP) supports the Security Assertion Markup Language (SAML) 2.0 or WS-Federation (WS-Fed) protocol. When you set up SAML/WS-Fed IdP federation with a partner's IdP, new guest users from that domain sign in to your Entra tenant with their own IdP-managed organizational account, then start collaborating with you. There's no need for the guest user to create a separate Entra account.
...
- You set up federation with an organization's SAML/WS-Fed IdP and invite guest users, and then the partner organization later moves to Entra ID. The guest users who have already redeemed invitations continue to use the federated SAML/WS-Fed IdP, as long as the federation policy in your tenant exists.
...
With SAML/WS-Fed IdP federation, guest users sign into your Entra tenant using their own organizational account. When they're accessing shared resources and are prompted for sign-in, users are redirected to their IdP. After successful sign-in, users are returned to Entra ID to access resources. If the Entra session expires or becomes invalid and the federated IdP has SSO enabled, the user experiences SSO. If the federated user's session is valid, the user isn't prompted to sign in again. Otherwise, the user is redirected to their IdP for sign-in.

### Security Assertion Markup Language 2.0 configuration

Entra B2B can be configured to federate with identity providers that use the SAML protocol with specific requirements listed below.

Note

You can associate multiple domains with a single federation configuration. The partner's domain can be either Entra verified or unverified.
...
Entra B2B can be configured to federate with identity providers that use the WS-Fed protocol with some specific requirements as listed below. Currently, the two WS-Fed providers that have been tested for compatibility with Entra ID are AD FS and Shibboleth.

The partner's domain can be either Entra verified or unverified. The partner's passive authentication URL domain must match the target domain, or a host within it. If it doesn't, the partner must add a DNS TXT record for the authentication URL domain to enable federation.
...
Federation with Google lets invited users sign in to your shared apps and resources with their own Gmail accounts, without having to create Microsoft accounts.
...
Google is deprecating embedded web-view sign-in support (Starting September 30, 2021). Google Gmail users won't be able to authenticate if your apps authenticate users with an embedded web-view and you use Google federation with Entra B2C or Entra B2B for external user invitations or self-service sign-up.
...
First, create a new project in the Google Developers Console to obtain a client ID and a client secret that you can later add to Entra ID.
...
3. Create a new project: On the dashboard, select **Create Project**, give the project a name (for example, **Entra B2B**), and then select **Create**:
 ...
 9. Select **Save**. 10. Select **Credentials**. On the **Create credentials** menu, select **OAuth client ID**:
  ...
  ​
 11. Under **Application type**, select **Web application**. Give the application a suitable name, like **Entra B2B**. Under **Authorized redirect URIs**, enter the following URIs:
...
### Step 2: Configure Google federation in Entra ID
...
1. Go to the [Azure portal](https://portal.azure.com/). On the left pane, select **Entra ID**.
...
**To delete Google federation in Entra ID**
...
- For **Valid OAuth redirect URIs**, enter `https://login.microsoftonline.com/te/ tenant-id /oauth2/authresp`. You can find your `tenant-ID`in the Entra ID Overview screen.
...
7. Choose a **Category**, for example Business and Pages. This value is required by Facebook, but not used for Entra ID.
...
18. To make your Facebook application available to Entra ID, select the Status selector at the top right of the page, turn it **On** to make the Application public, then select **Switch Mode**. At this point, the Status should change from **Development** to **Live**.

### Configure a Facebook account as an identity provider

Now you'll set the Facebook client ID and client secret, either by entering it in Entra admin center or by using PowerShell. You can test your Facebook configuration by signing up via a user flow on an app enabled for self-service sign-up.

#### To configure Facebook federation in the Entra ID screen

1. Sign in to the [Azure portal](https://portal.azure.com/) as the global administrator of your Entra tenant.
...
#### To delete Facebook federation in Entra ID:

1. Go to the [Azure portal](https://portal.azure.com/). In the left pane, select **Entra ID**.
2. Select **External Identities**.
3. Select **All identity providers**.
...
Entra organizations can use external identities cross-tenant access settings to manage how they collaborate with other Entra organizations or Microsoft clouds. Cross-tenant access settings give you granular control over how external Entra organizations collaborate with you, **inbound access**. You can also control how your users collaborate with external Entra organizations, **outbound access**.

#### Manage inbound and outbound settings

By default, B2B collaboration with other Entra organizations is enabled, and B2B direct connect is blocked. But the following comprehensive admin settings let you manage both of these features.
...
| Inbound access settings | Control whether users from external Entra organizations can access resources in your organization. You can apply these settings to everyone, or specify individual users, groups, and applications. |
| Trust settings (inbound) | Determine whether your Conditional Access policies will trust the multifactor authentication (MFA). You can also require compliant device, and hybrid Entra joined device. And finally, allow or restrict user from an external organization if their users have already satisfied these requirements in their home tenants. |
| B2b direct connect | Set up a mutual trust relationship with another Entra organization for seamless collaboration. This feature currently works with Microsoft Teams shared channels. |
...
B2B direct connect requires a mutual trust relationship between two Entra organizations to allow access to each other's resources. Both the resource organization and the external organization need to mutually enable B2B direct connect in their cross-tenant access settings. When the trust is established, the B2B direct connect user has single sign-on access to resources outside their organization using credentials from their home Entra organization.

Currently, B2B direct connect capabilities work with Teams shared channels. Once B2B direct connect is established between two organizations, a user in one organization can create a shared channel in Teams and invite an external B2B direct connect user to it. From within Teams, the B2B direct connect user then accesses that shared channel seamlessly in their home tenant Teams instance, without manually signing in to the organization hosting it.
...
- [What is a guest user access in Entra ID](https://learn.microsoft.com/en-us/entra/external-id/user-properties)
- [Set up the Entra Verified ID service](https://learn.microsoft.com/en-us/entra/verified-id/verifiable-credentials-configure-tenant)
- [Cross-tenant access with Entra External Identities](https://learn.microsoft.com/en-us/entra/architecture/external-identity-deployment-architectures)
...
- Plan, design, and implement Entra Connect
...
- Implement and manage Entra Connect Health
...
Entra Connect is a solution that bridges an organizations on-premises Active Directory with your cloud-based Entra ID. IT can synchronize identities from on-premises into Azure and ensures a consistent identity across both platforms. This connection enables services like password hash synchronization, pass-through authentication, and seamless single sign-on (SSO).

Entra Connect is the Microsoft tool designed to meet and accomplish your hybrid identity goals. It provides the following capabilities:
...
- Password hash synchronization - A sign-in method that synchronizes a hash of a user's on-premises AD password with Entra ID.
...
- Federation integration - Federation is an optional part of Entra Connect and can be used to configure a hybrid environment using an on-premises AD FS infrastructure. It also provides AD FS management capabilities such as certificate renewal and more AD FS server deployments.
- Health monitoring - Entra Connect-Health provides robust monitoring.

### Why use Entra Connect?

Integrating your on-premises directories with Entra ID makes your users more productive by providing a common identity for accessing both cloud and on-premises resources. With Entra Connect, users can use a single identity to access on-premises applications and cloud services such as Microsoft 365. Additionally, organizations can provide an easy deployment experience for synchronization and sign-in using a single tool. Entra Connect replaces older versions of identity integration tools; and is included in your Entra ID subscription.

### Select an authentication method

Identity is the new control plane of IT security, so authentication is an organization’s access guard to the new cloud world. Organizations need an identity control plane that strengthens their security and keeps their cloud apps safe from intruders. When the Entra hybrid identity solution is your new control plane, authentication is the foundation of cloud access. Choosing the correct authentication method is a crucial first decision in setting up an Entra hybrid identity solution. To choose an authentication method, you need to consider the time, existing infrastructure, complexity, and cost of implementing your choice. These factors are different for every organization and might change over time.

#### Cloud authentication

When you choose this authentication method, Entra ID handles users' sign-in process. When you couple with seamless single sign-on (SSO), users can sign into cloud apps without having to reenter their credentials. With cloud authentication, you can choose from two options:

**Entra password hash synchronization (PHS)**. The simplest way to enable authentication for on-premises directory objects in Entra. Users can use the same username and password that they use on-premises without having to deploy any more infrastructure.

- **Effort** - Password hash synchronization requires the least effort regarding deployment, maintenance, and infrastructure. This level of effort typically applies to organizations that only need their users to sign in to Microsoft 365, SaaS apps, and other Entra ID-based resources. When turned on, password hash synchronization is part of the Entra Connect sync process and runs every two minutes.
...
- **Advanced scenarios** - If organizations choose to, it's possible to use insights from identities with Entra Identity Protection reports with Entra ID Premium P2. An example is the leaked credentials report. Windows Hello for Business has specific requirements when you use password hash synchronization. Entra Domain Services requires password hash synchronization to create users with their corporate credentials in the managed domain.
- **Business continuity** - Using password hash synchronization with cloud authentication is highly available as a cloud service that scales to all Microsoft datacenters. To make sure password hash synchronization doesn't go down for extended periods, deploy a second Entra Connect server in staging mode in a standby configuration.
- **Considerations** - Currently, password hash synchronization doesn't immediately enforce changes in on-premises account states. In this situation, a user has access to cloud apps until the user account state is synchronized to Entra ID. Organizations might want to overcome this limitation by running a new synchronization cycle after administrators do bulk updates to on-premises user account states. An example is disabling accounts.

**Entra pass-through authentication (PTA)**. Provides a simple password validation for Entra authentication services by using a software agent that runs on one or more on-premises servers. The servers validate the users directly with your on-premises Active Directory, which ensures that the password validation doesn't happen in the cloud. Companies with a security requirement to immediately enforce on-premises user account states, password policies, and sign in hours might use this authentication method.
...
- **Business continuity** - We recommend that you deploy two extra pass-through authentication agents. These extras are in addition to the first agent on the Entra Connect server. This deployment ensures high availability of authentication requests. When you have three agents deployed, one agent can still fail when another agent is down for maintenance.
- **Considerations** - You can use password hash synchronization as a backup authentication method for pass-through authentication, for when a significant on-premises failure stops the agents from validating a user's credentials. Fail over to password hash synchronization doesn't happen automatically and you must use Entra Connect to switch the sign-in method manually.

#### Federated authentication

With this authentication method, Entra ID hands the authentication process to a separate trusted authentication system, such as on-premises Active Directory Federation Services (AD FS), which validates the user’s password. The authentication system can provide other advanced authentication requirements. Examples are smartcard-based authentication or third party multifactor authentication.

- **Effort** - A federated authentication system relies on an external trusted system to authenticate users. Some companies want to reuse their existing federated system investment with their Entra hybrid identity solution. The maintenance and management of the federated system falls outside the control of Entra ID. It's up to the organization by using the federated system to make sure it's deployed securely and can handle the authentication load.
...
- **Advanced scenarios** - A federated authentication solution is required when customers have an authentication requirement that Entra ID doesn't support natively.
...
The following diagrams outline the high-level architecture components required for each authentication method you can use with your Entra hybrid identity solution. They provide an overview to help you compare the differences between the solutions.
...
- **Identity protection** - One of the best ways to protect users in the cloud is Entra Identity Protection with Entra Premium P2. Microsoft continually scans the Internet for user and password lists that bad actors sell and make available on the dark web. Entra ID can use this information to verify if any of the usernames and passwords in your organization are compromised. Therefore, it's critical to enable password hash synchronization no matter which authentication method you use, whether it's federated or pass-through authentication. Leaked credentials are presented as a report. Use this information to block or force users to change their passwords when they try to sign in with leaked passwords.

### Entra Connect design concepts

This section describes areas that must be thought through during the implementation design of Entra Connect. It's a deep dive on certain areas and these concepts are briefly described in other documents as well.

### sourceAnchor

The sourceAnchor attribute is defined as *an attribute immutable during the lifetime of an object*. It uniquely identifies an object as being the same object on-premises and in Entra ID. The attribute is also called **immutableId** and the two names are used interchangeable. The attribute is used for the following scenarios:

- When a new sync engine server is built, or rebuilt after a disaster recovery scenario, this attribute links existing objects in Entra ID with objects on-premises.
- If you move from a cloud-only identity to a synchronized identity model, then this attribute allows objects to "hard match" existing objects in Entra ID with on-premises objects.
...
If you have a single forest on-premises, the attribute you should use is **objectGuid**. You can also use the objectGuid attribute when you use express settings in Entra Connect. And also the attribute used by DirSync. If you have multiple forests and don't move users between forests and domains, then **objectGUID** is a good attribute to use. Another solution is to pick an existing attribute you know doesn't change. Commonly used attributes include **employeeID**. If you consider an attribute that contains letters, make sure there's no chance the case (upper case vs. lower case) can change for the attribute's value. Bad attributes include those attributes with the name of the user. Once the sourceAnchor attribute is decided, the wizard stores the information in your Entra tenant. The information will be used by future installation of Entra Connect.

### Entra sign-in

The synchronization settings of your on-premises directory integration with Entra ID can affect the way user authenticates. Entra uses userPrincipalName (UPN) to authenticate the user. However, when you synchronize your users, you must choose the attribute to be used for value of userPrincipalName carefully. When you're selecting the attribute for providing the value of UPN to be used in Azure one should ensure

- The attribute values conform to the UPN syntax (RFC 822), the format username@domain
- The suffix in the values matches to one of the verified custom domains in Entra ID
...
Ensure that there's a verified domain for the User Principal Name (UPN) suffix. John is a user in contoso.com. You want John to use the on-premises UPN john@contoso.com to sign in to Azure after you have synced users to your Entra directory contoso.onmicrosoft.com. To do so, you need to add and verify contoso.com as a custom domain in Entra ID before you can start syncing the users. If the UPN suffix of John, for example contoso.com, doesn't match a verified domain in Entra ID, then the tool replaces the UPN suffix with contoso.onmicrosoft.com.

Some organizations have non-routable domains, like contoso.local, or simple single label domains like contoso. You're not able to verify a non-routable domain. Entra Connect can sync to only a verified domain in Entra ID. When you create an Entra directory, it creates a routable domain that becomes default domain for your Entra ID for example, contoso.onmicrosoft.com. Therefore, it becomes necessary to verify any other routable domain in such a scenario in case you don't want to sync to the default onmicrosoft.com domain.

Entra Connect detects if you're running in a non-routable domain environment and would appropriately warn you from going ahead with express settings. If you're operating in a non-routable domain, then it's likely that the UPN, of the users, has a non-routable suffix too. For example, if you're running under contoso.local, Entra Connect suggests you use custom settings rather than using express settings. Using custom settings, you're able to specify the attribute that should be used as UPN to sign in to Azure after the users are synced to Entra ID.

### Topologies for Entra Connect

This section describes various on-premises and Entra ID topologies that use Entra Connect sync as the key integration solution; it includes both supported and unsupported configurations.

| **Common topology** | **Description** |
|---|---|
| Single forest, single Entra tenant | The most common topology is a single on-premises forest, with one or multiple domains, and a single Entra tenant. For authentication, password hash synchronization is used. The express installation of Entra Connect supports only this topology. |
| Multiple forests, single Entra tenant | Many organizations have environments with multiple on-premises Active Directory forests. There are various reasons for having more than one on-premises Active Directory forest. Typical examples are designs with account-resource forests and the result of a merger or acquisition. When you have multiple forests, all forests must be reachable by a single Entra Connect sync server. The server must be joined to a domain. If necessary to reach all forests, you can place the server in a perimeter network (also known as DMZ, demilitarized zone, and screened subnet). |
| Multiple forests, single sync server, users are represented in only one directory | In this environment, all on-premises forests are treated as separate entities. No user is present in any other forest. Each forest has its own Exchange organization, and there's no GALSync between the forests. This topology might be the situation after a merger/acquisition or in an organization where each business unit operates independently. These forests are in the same organization in Entra ID and appear with a unified GAL. In the preceding picture, each object in every forest is represented once in the metaverse and aggregated in the target tenant. |
| Multiple forests: full mesh with optional GALSync | A full mesh topology allows users and resources to be located in any forest. Commonly, there are two-way trusts between the forests. If Exchange is present in more than one forest, there might be (optionally) an on-premises GALSync solution. Every user is then represented as a contact in all other forests. GALSync is commonly implemented through FIM 2010 or MIM 2016. Entra Connect can't be used for on-premises GALSync. |
...
| Staging server | Entra Connect supports installing a second server in *staging mode*. A server in this mode reads data from all connected directories but doesn't write anything to connected directories. It uses the normal synchronization cycle and therefore has an updated copy of the identity data. |
| Multiple Entra tenants | There's a 1:1 relationship between an Entra Connect sync server and a tenant. For each Entra tenant, you need one Entra Connect sync server installation. The AD tenant instances are isolated by design. That is, users in one tenant can't see users in the other tenant. Separation of users is a supported configuration. Otherwise, you should use the single Entra tenant model. |
| Each object only once in an Entra tenant | In this topology, one Entra Connect sync server is connected to each tenant. The Entra Connect sync servers must be configured for filtering so that each has a mutually exclusive set of objects to operate on. You can, for example, scope each server to a particular domain or organizational unit. |

### Entra Connect component factors
...
![Diagram of how the connected directories and Entra Connect provisioning engine interact. Includes Connector Space and Metaverse components in an SQL Database.](https://learn.microsoft.com../../wwl-sci/implement-manage-hybrid-identity/media/azure-active-directory-connect-internal.png)

The provisioning engine connects to each Active Directory forest and to Entra ID. The process of reading information from each directory is called Import. Export refers to updating the directories from the provisioning engine. Sync evaluates the rules of how the objects will flow inside the provisioning engine.

Entra Connect uses the following staging areas, rules, and processes to allow the sync from Active Directory to Entra ID:

- **Connector Space (CS)** - Objects from each connected directory (CD), the actual directories, are staged here first before they can be processed by the provisioning engine. Entra ID has its own CS and each forest you connect to will have its own CS.
...
Entra Connect cloud sync is designed to accomplish hybrid identity goals for synchronization of users, groups and contacts to Entra ID. The synchronization is accomplished by using the **cloud provisioning agent** instead of the Entra Connect application. It can be used alongside Entra Connect sync and it provides the following benefits:

- Support for synchronizing to an Entra tenant from a multi-forest disconnected Active Directory forest environment: The common scenarios include merger and acquisition. The acquired company's AD forests are isolated from the parent company's AD forests. The companies that have historically had multiple AD forests.
- Simplified installation with light-weight provisioning agents: The agents act as a bridge from AD to Entra ID, with all the sync configuration managed in the cloud.
- Multiple provisioning agents can be used to simplify high availability deployments, critical for organizations relying upon password hash synchronization from AD to Entra ID.
...
With Entra Connect cloud sync, provisioning from AD to Entra ID is orchestrated in Microsoft Online Services. An organization only needs to deploy, in their on-premises or IaaS-hosted environment, a light-weight agent that acts as a bridge between Entra ID and AD. The provisioning configuration is stored and managed as part of the service. Reminder that the sync runs every 2 minutes.
...
Password hash synchronization is one of the sign-in methods used to accomplish hybrid identity. Entra Connect synchronizes a hash, of the hash, of a user's password from an on-premises Active Directory instance to a cloud-based Entra instance.

![Diagram of Entra Connect passes a password hash for a user between on-premises and in the cloud.](https://learn.microsoft.com../../wwl-sci/implement-manage-hybrid-identity/media/password-hash-sync-architecture.png)

Active Directory Domain Services stores passwords in the form of a hash value representation of the actual user password. A hash value is a result of a one-way mathematical function (the hashing algorithm). There's no method to revert the result of a one-way function to the plain text version of a password. To synchronize your password, Entra Connect sync extracts your password hash from the on-premises Active Directory instance. Extra security processing is applied to the password hash before it's synchronized to the Entra authentication service. Passwords are synchronized on a per-user basis and in chronological order.
...
When you install Entra Connect by using the **Express Settings** option, password hash synchronization is automatically enabled. If you use custom settings when you install Entra Connect, password hash synchronization is available on the user sign-in page.
...
Configure PingFederate with Entra Connect to set up federation with the domain you want connected. The following prerequisites are required:
...
Configure PingFederate as the federation server for each federated Azure domain. Then select Export Settings to share this information with your PingFederate administrator. The federation server administrator updates the configuration and provides the PingFederate server URL and port number so that Entra Connect can verify the metadata settings.


## Implement manage pass-through authentication (PTA)

Entra pass-through authentication allows your users to sign in to both on-premises and cloud-based applications by using the same passwords. Pass-through authentication signs users in by validating their passwords directly against on-premises Active Directory.

### Enable the feature

Enable pass-through authentication through [Entra Connect](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/whatis-hybrid-identity).

If you're installing Entra Connect for the first time, choose the [custom installation path](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-install-custom). At the **User sign-in** page, choose **Pass-through authentication** as the **Sign On method**. On successful completion, a pass-through authentication agent is installed on the same server as Entra Connect. In addition, the pass-through authentication feature is enabled on your tenant.

If you already installed Entra Connect with the express or the custom installation path, select the **Change user sign-in** task on Entra Connect, then select **Next**. Then select **Pass-through authentication** as the sign-in method. On successful completion, a pass-through authentication agent is installed on the same server as Entra Connect and the feature is enabled on your tenant.
...
Entra seamless single sign-on (seamless SSO) automatically signs in users from their network-connected corporate desktops. Seamless SSO provides your users with easy access to cloud-based applications without needing any other on-premises components.
...
2. If the user isn't already signed in, the user is redirected to the Entra sign-in page.
3. The user types in their user name into the Entra sign-in page.
4. Using JavaScript in the background, Entra ID challenges the browser, via a 401 Unauthorized response, to provide a Kerberos ticket.
5. The browser, in turn, requests a ticket from Active Directory for the AZUREADSSOACC computer account (which represents Entra ID).
...
7. The browser forwards the Kerberos ticket it acquired from Active Directory to Entra ID.
8. Entra ID decrypts the Kerberos ticket, which includes the identity of the user signed into the corporate device, using the previously shared key.
9. After evaluation, Entra ID either returns a token back to the application or asks the user to perform additional proofs, such as multifactor authentication.
...
3. The app sends the username to Entra ID, and retrieves your tenant's WS-Trust MEX endpoint. This WS-Trust endpoint is used exclusively by the Seamless SSO feature, and isn't a general implementation of the WS-Trust protocol on Entra ID.
...
6. If the app is able to retrieve the Kerberos ticket, it forwards it up to Entra integrated authentication endpoint.
7. Entra ID decrypts the Kerberos ticket and validates it.
8. Entra ID signs the user in, and issues a SAML token to the app.
9. The app then submits the SAML token to Entra ID OAuth2 token endpoint.
10. Entra ID validates the SAML token and issues an access token, a refresh token for the specified resource, and an ID token to the app.
11. The user gets access to the app's resource.


## Implement and manage federation

Federation can use a new or existing on-premises Active Directory farm in Windows Server 2012 R2 (or later). With Entra Connect, users then log into Entra resources using their on-premises password.

![Diagram of federation between on-premises and Entra ID. Shows users able log into both on-premises and cloud resources with a single shared sign in.](https://learn.microsoft.com../../wwl-sci/implement-manage-hybrid-identity/media/sc300-federation-flow-diagram.png)
...
You can federate your on-premises environment with Entra ID and use this federation for authentication and authorization. This sign-in method ensures that all user authentication occurs on-premises. This method allows administrators to implement more rigorous levels of access control. Federation with AD FS and PingFederate is available.

With federated sign-in, your users can sign in to Entra based services with their on-premises passwords. While they're on the corporate network, they don't even have to enter their passwords. By using the federation option with AD FS, you can deploy a new or existing farm with AD FS in Windows Server 2012 R2 or later. If you choose to specify an existing farm, Entra Connect configures the trust between your farm and Entra ID so that your users can sign in.

### Requirement to deploy federation with AD FS and Entra Connect
...
- The machine you run the wizard on must be able to connect, using Windows Remote Management, to any other machine where you want to install AD FS or Web Application Proxy.

### Set up your federation using Entra Connect to connect to an AD FS farm

**Specify the AD FS servers** - Specify the servers where you want to install AD FS. You can add one or more servers, depending on your capacity needs. Before you set up this configuration, join all AD FS servers to Active Directory. This step isn't required for the Web Application Proxy servers. Microsoft recommends installing a single AD FS server for test and pilot deployments. After the initial configuration, you can add and deploy more servers to meet your scaling needs by running Entra Connect again.

**Specify the Web Application Proxy servers** - Specify your Web Application Proxy servers. The Web Application Proxy server is deployed in your perimeter network, facing the extranet. It supports authentication requests from the extranet. You can add one or more servers, depending on your capacity needs. After the initial configuration, you can add and deploy more servers to meet your scaling needs by running Entra Connect again.

**Specify the service account for the AD FS service** - The AD FS service needs a domain service account to authenticate users and look up user information in Active Directory. It can support two types of service accounts:

- Group managed service account
- Domain user account

**Select the Entra domain that you want to federate** - Use the Entra domain page to set up the federation relationship between AD FS and Entra ID. Here, you configure AD FS to provide security tokens to Entra ID. You also configure Entra ID to trust the tokens from this AD FS instance. On this page, you can configure only a single domain in the initial installation. You can configure more domains later by running Entra Connect again.

### Entra Connect tools to manage your federation

You can complete various AD FS-related tasks in Entra Connect with minimal user intervention by using the Entra Connect wizard. Even after you've finished installing Entra Connect by running the wizard, you can run the wizard again to do other tasks. For example, you can use the wizard to repair the trust with Microsoft 365, to federate with Entra ID using an alternate sign in ID, and to add an AD FS Web Application Proxy (WAP) server.

**Repair the trust** - Entra Connect can check the current health of the AD FS and Entra ID trust, then take the appropriate actions to repair it.

**Federate with Entra ID using AlternateID** - It's recommended that the on-premises User Principal Name(UPN) and the cloud User Principal Name are kept the same. If the on-premises UPN uses a non-routable domain (ex. Contoso.local) or can't be changed due to local application dependencies, we recommend setting up alternate sign in ID. Alternate sign in ID allows you to configure a sign-in experience where users can sign in with an attribute other than their UPN, such as mail. The choice for User Principal Name in Entra ID Connect defaults to the userPrincipalName attribute in Active Directory. If you choose any other attribute for User Principal Name and are federating using AD FS, then Entra Connect will configure AD FS for alternate sign in ID.

**Add a federated domain** - It's easy to add a domain to be federated with Entra ID by using Entra Connect. Entra Connect adds the domain for federation and modifies the claim rules to correctly reflect the issuer when you have multiple domains federated with Entra ID.
...
- Set necessary permissions on the Entra Connector account, to manage devices on your Active Directory.


## Trouble-shoot synchronization errors

Errors could occur when identity data is synchronized from Windows Server Active Directory (AD DS) to Entra ID. This section provides an overview of different types of sync errors, some of the possible scenarios that cause those errors and potential ways to fix the errors. This section includes the common error types, but doesn't cover all the possible errors.

With the latest version of Entra Connect, a report of Synchronization Errors is available in the [Azure portal](https://aka.ms/aadconnecthealth) as part of Entra Connect Health for sync.

Entra Connect performs three types of operations from the directories it keeps in sync: Import, Synchronization, and Export. Errors can take place in all the operations. This section mainly focuses on errors during Export to Entra ID.

### Errors during export to Entra ID

The following section describes different types of synchronization errors that can occur during the export operation to Entra ID using the Entra connector. This connector can be identified by the name format being `contoso.onmicrosoft.com`. Errors during export to Entra ID indicate that the operation (add, update, delete etc.,) attempted by Entra Connect (Sync Engine) on Entra directory failed.
...
- When Entra Connect (sync engine) instructs the directory to add or update objects, Entra ID matches the incoming object's **sourceAnchor** attribute to the **immutableId** attribute of objects in Entra ID. This match is called a **Hard Match**.
- When Entra ID **doesn't find** any object whose **immutableId** attribute matches the **sourceAnchor** attribute of the incoming object, it doesn't provision a new object right away. It first falls back to the ProxyAddresses and UserPrincipalName attributes to find a match. This match is called a **Soft Match**. The Soft Match matches objects already present in Entra ID with new objects added or updated during synchronization that represent the same on-premises entity (users, groups).
- **InvalidSoftMatch** error occurs when the hard match finds no matching object **AND** the soft match does find one, but that object's *immutableId* differs from the incoming object's *SourceAnchor*. This suggests that the matching object was synchronized with another object from on premises Active Directory.
...
Entra directory schema doesn't allow two or more objects to have the same value of the following attributes. (This isn't an exhaustive list.)
...
[Entra Attribute Duplicate Attribute Resiliency](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-syncservice-duplicate-attribute-resiliency) feature is also being rolled out as the default behavior of Entra ID. This makes Entra ID more resilient in the way it handles duplicated ProxyAddresses and UserPrincipalName attributes present in on premises AD environments, which reduces the number of synchronization errors seen by Entra Connect and other sync clients. This feature doesn't fix the duplication errors. So the data still needs to be fixed. But it allows provisioning of new objects which are otherwise blocked from being provisioned due to duplicated values in Entra ID. This will also reduce the number of synchronization errors returned to the synchronization client. If this feature is enabled for your Tenant, you won't see the InvalidSoftMatch synchronization errors seen during provisioning of new objects.

**Example scenarios for InvalidSoftMatch**

- Two or more objects with the same value for the ProxyAddresses attribute exist in on-premises Active Directory. Only one is getting provisioned in Entra ID.
- Two or more objects with the same value for the userPrincipalName attribute exist in on-premises Active Directory. Only one is getting provisioned in Entra ID.
- An object was added in the on premises Active Directory with the same value of ProxyAddresses attribute as that of an existing object in Entra directory. The object added on premises isn't getting provisioned in Entra directory.
- An object was added in on premises Active Directory with the same value of userPrincipalName attribute as that of an account in Entra ID. The object isn't getting provisioned in Entra ID.
- A synced account was moved from Forest A to Forest B. Entra Connect (sync engine) was using ObjectGUID attribute to compute the SourceAnchor. After the forest move, the value of the SourceAnchor is different. The new object (from Forest B) is failing to sync with the existing object in Entra ID.
- A synced object was accidentally deleted from on premises Active Directory, then a new object was created in Active Directory for the same entity (such as user), without deleting the account in Entra ID. The new account fails to sync with the existing Entra object.
- Entra Connect was uninstalled and reinstalled. During the reinstallation, a different attribute was chosen as the SourceAnchor. All the objects that had previously synced stopped syncing with InvalidSoftMatch error.

**Example case:**

1. **Bob Smith** is a synced user in Entra ID from on premises Active Directory of *contoso.com*
2. Bob Smith's **UserPrincipalName** is set as **bobs@contoso.com**.
3. **"abcdefghijklmnopqrstuv=="** is the **SourceAnchor** calculated by Entra Connect using Bob Smith's **objectGUID** from on premises Active Directory, which is the **immutableId** for Bob Smith in Entra ID.
...
3. **"abcdefghijkl0123456789==""** is the **sourceAnchor** calculated by Entra Connect using Bob Taylor's **objectGUID** from on premises Active Directory. Bob Taylor's object has NOT synced to Entra ID yet.
...
1. During sync, Entra Connect will recognize the addition of Bob Taylor in on premises Active Directory and ask Entra ID to make the same change.
2. Entra ID will first perform hard match. That is, it will search if there's any object with the immutableId equal to "abcdefghijkl0123456789==". Hard Match will fail, since no other object in Entra ID will have that immutableId.
3. Entra ID will then attempt to soft-match Bob Taylor. That is, it will search if there's any object with proxyAddresses equal to the three values, including smtp: bob@contoso.com
4. Entra ID will find Bob Smith's object to match the soft-match criteria. But this object has the value of immutableId = "abcdefghijklmnopqrstuv==". which indicates this object was synced from another object from on premises Active Directory. Thus, Entra ID can't soft-match these objects and results in an **InvalidSoftMatch** sync error.

**How to fix InvalidSoftMatch error**

The most common reason for the InvalidSoftMatch error is two objects with different SourceAnchor (immutableId) sharing the same value for the ProxyAddresses and/or UserPrincipalName attributes, which Entra ID uses during the soft-match process. In order to fix the Invalid Soft-Match

1. Identify the duplicated proxyAddresses, userPrincipalName, or other attribute value that's causing the error. Also identify which two (or more) objects are involved in the conflict. The report generated by [Entra Connect Health for sync](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-health-sync) can help you identify the two objects.
...
4. If you made the change in the on premises AD, let Entra Connect sync the change.

Sync error reports within Entra Connect Health for sync are updated every 30 minutes and include the errors from the latest synchronization attempt.

Note

ImmutableId, by definition, shouldn't change in the lifetime of the object. Entra Connect might not have been configured with some of the scenarios in the above list in mind. It can then calculate a different SourceAnchor value for the AD object than the one on the existing Entra Object you wish to continue using, even though both represent the same entity (same user/group/contact etc.,).

#### ObjectTypeMismatch

**Description**

When Entra ID attempts to soft match two objects, those objects can be of a different "object type" (such as User, Group, Contact etc.) yet still have the same values for the attributes used to perform the soft-match. As duplication of these attributes isn't permitted in Entra, the operation can result in "ObjectTypeMismatch" synchronization error.

**Example scenarios for ObjectTypeMismatch error**

- A mail enabled security group is created in Microsoft 365. Admin adds a new user or contact in on premises AD, not yet synchronized to Entra ID, whose ProxyAddresses attribute has the same value as that of the Microsoft 365 group.
...
3. When Entra Connect will sync the new user account, it will get the "ObjectTypeMismatch" error.
...
2. Identify which object should continue to have the duplicated value and which object shouldn't.
3. Remove the duplicated value from the object that shouldn't have that value. You should make the change in the directory where the object is sourced from. In some cases, you need to delete one of the objects in conflict.
4. If you made the change in the on premises AD, let Entra Connect sync the change. Sync error report within Entra Connect Health for sync gets updated every 30 minutes and includes the errors from the latest synchronization attempt.
...
Entra schema doesn't allow two or more objects to have the same value of the following attributes. That is each object in Entra ID is forced to have a unique value of these attributes at a given instance.

- ProxyAddresses
- UserPrincipalName

If Entra Connect adds a new object, or updates an existing one, with a value for the above attributes that already belongs to another object in Entra ID, the operation fails with the "AttributeValueMustBeUnique" sync error.
...
4. Bob Taylor's object is synchronized with Entra ID successfully.
5. Admin decided to update Bob Taylor's **ProxyAddresses** attribute with the following value: i. **smtp: bob@contoso.com**
6. Entra ID will attempt to update Bob Taylor's object with the above value, but that operation fails with the "AttributeValueMustBeUnique" error, because the ProxyAddresses value is already assigned to Bob Smith.
...
2. Identify which object should continue to have the duplicated value and which object shouldn't.
3. Remove the duplicated value from the object that shouldn't have that value. You should make the change in the directory where the object is sourced from. In some cases, you need to delete one of the objects in conflict.
4. If you made the change in the on premises AD, let Entra Connect sync the change for the error to get fixed.
...
Entra ID enforces various restrictions on the data itself before allowing that data to be written into the directory. These restrictions are to ensure that end users get the best possible experiences while using the applications that depend on this data.
...
3. Both contoso.com and fabrikam.com domains are federated domains with Entra ID.
...
When an attribute exceeds the allowed size limit, length limit or count limit set by Entra schema, the synchronization operation results in the **LargeObject** or **ExceededAllowedLength** sync error. Typically this error occurs for the following attributes
...
3. Bob's thumbnailPhoto set in Active Directory is too large to be synced in Entra ID.
...
- the same UserPrincipalName as an existing Entra object

Entra Connect isn't allowed to soft-match a user object from on-premises AD with a user object in Entra ID that has an administrative role assigned to it.

#### How to fix

To resolve this issue do the following:

1. Remove the Entra account (owner) from all admin roles.
...
You can assign the administrative role to the existing user object again after the soft-match between the on-premises user object and the Entra user object has completed.


## Implement Entra Connect Health

Entra Connect Health provides monitoring of your on-premises identity infrastructure. It enables you to maintain a reliable connection to Microsoft 365 and Microsoft Online Services. This reliability is achieved by providing monitoring capabilities for your key identity components. Also, it makes the key data points about these components easily accessible.

The information is presented in the [Entra Connect Health portal](https://aka.ms/aadconnecthealth). Use the Entra Connect Health portal to view alerts, performance monitoring, usage analytics, and other information. Entra Connect Health enables the single lens of health for your key identity components in one place.

![Diagram of Entra Connect Health. Shows how Entra Connect is maintained.](https://learn.microsoft.com../../wwl-sci/implement-manage-hybrid-identity/media/azure-active-directory-connect-health-2.png)

Using the Entra Connect Health feature requires an Entra ID Premium P1 license.

### Entra Connect Health agent installation

This section provides instructions for installing and configuring the Entra Connect Health agents.

### Requirements

- Entra ID Premium is installed.
- You're a global administrator in Entra ID.
- The Entra Connect Health agent is installed on each targeted server.
...
- Firewall ports on the server are running the agent.
  - The agent requires the following firewall ports to be open so that it can communicate with the Entra Connect Health service endpoints:
    - TCP port 443
    - TCP port 5671
...
Download and install the Entra Connect Health agent from the Download Center.
...
A PowerShell window opens to start the agent registration process. When you're prompted, sign in by using an Entra ID account that has permissions to register the agent. By default, the global admin account has permissions.
...
- Entra Connect Health AD FS Insights Service
- Entra Connect Health AD FS Monitoring Service

### Install the agent for Sync

The Entra Connect Health agent for Sync is installed automatically in the latest version of Entra Connect. To use Entra Connect for Sync, download the latest version of Entra Connect and install it.
...
Remember that you must have Entra ID Premium to use Entra Connect Health. If you don't have Entra ID Premium, you can't complete the configuration in the Azure portal.


## Manage Entra Health

This section describes various operations you can perform by using Entra Connect Health.

### Enable email notifications

You can configure the Entra Connect Health service to send email notifications when alerts indicate that your identity infrastructure isn't healthy. This occurs when an alert is generated, and when it's resolved.

Note

Email notifications are enabled by default.

#### To enable Entra Connect Health email notifications
...
When our back-end service hits issues processing synchronization requests, it sends a notification email with the error details to the administrative contact email address(es) of your tenant. We heard feedback from customers that in certain cases the volume of these messages is prohibitively large so we're changing the way we send these messages.
...
Entra ID premium license is required for the deletion steps.

In some instances, you might want to remove a server from being monitored. Here's what you need to know to remove a server from the Entra Connect Health service.
...
#### Delete a server from the Entra Connect Health service

Note

Entra ID premium license is required for the deletion steps.

Entra Connect Health for Active Directory Federation Services (AD FS) and Entra Connect (Sync):
...
Entra Connect Health for Entra Domain Services:
...
#### Delete a service instance from Entra Connect Health service

In some instances, you might want to remove a service instance. Here's what you need to know to remove a service instance from the Entra Connect Health service.
...
#### To delete a service instance from the Entra Connect Health service
...
Azure role-based access control (Azure RBAC) for Entra Connect Health provides access to users and groups. Azure RBAC assigns roles to the intended users and groups, and provides a mechanism to share administrator duty within your directory. Always thing of the principle of least privilege when assigning access.

#### Roles

Entra Connect Health supports the following built-in roles:

| **Role** | **Permissions** |
|---|---|
| Owner | Owners can *manage access* (for example, assign a role to a user or group), *view all information* (for example, view alerts) from the portal, and *change settings* (for example, email notifications) within Entra Connect Health. By default, Entra global administrators are assigned this role, and this can't be changed. |
| Contributor | Contributors can *view all information* (for example, view alerts) from the portal, and *change settings* (for example, email notifications) within Entra Connect Health. |
| Reader | Readers can *view all information* (for example, view alerts) from the portal within Entra Connect Health. |

All other roles, such as User Access Administrators or DevTest Labs Users, have no impact on access within Entra Connect Health, even if the roles are available in the portal experience.

#### Access scope

Entra Connect Health supports managing access at two levels:

- **All service instances**: This is the recommended path in most cases. It controls access for all service instances (for example, an AD FS farm) across all role types that are being monitored by Entra Connect Health.
...
#### Allow users or groups access to Entra Connect Health

The following steps show how to allow access.

**Step 1: Select the appropriate access scope**

To allow a user access at the *all service instances* level within Entra Connect Health, open the main screen in Entra Connect Health.
...
- The Invite Users feature isn't supported within Entra Connect Health.

**Step 3: Share the location with users or groups**

1. After you assign permissions, a user can access Entra Connect Health by going [here](https://aka.ms/aadconnecthealth).
...
You can remove a user or a group added to Entra Connect Health and Azure RBAC. Select the user or group with the secondary action, and select **Remove**.

### Diagnose and remediate duplicated attribute sync errors

### Overview

Taking one step farther to highlight sync errors, Entra Connect Health introduces self-service remediation. It troubleshoots duplicated attribute sync errors and fixes objects that are orphaned from Entra ID. The diagnosis feature has these benefits:

- It provides a diagnostic procedure that narrows down duplicated attribute sync errors. And it gives specific fixes.
- It applies a fix for dedicated scenarios from Entra ID to resolve the error in a single step.
...
When **QuarantinedAttributeValueMustBeUnique** and **AttributeValueMustBeUnique** sync errors happen, it's common to see a **UserPrincipalName** or **Proxy Addresses** conflict in Entra ID. You might solve the sync errors by updating the conflicting source object from the on-premises side. The sync error will be resolved after the next sync. For example, this image indicates that two users have a conflict of their **UserPrincipalName**. Both are **Joe.J@contoso.com**. The conflicting objects are quarantined in Entra ID.
...
Occasionally, you might find that an existing user loses the **Source Anchor**. The deletion of the source object happened in on-premises Active Directory. But the change of deletion signal never got synchronized to Entra ID. This loss happens for reasons like sync engine issues or domain migration. When the same object gets restored or recreated, logically, an existing user should be the user to sync from the **Source Anchor**.

When an existing user is a cloud-only object, you can also see the conflicting user synchronized to Entra ID. The user can't be matched in sync to the existing object. There's no direct way to remap the **Source Anchor**.

As an example, the existing object in Entra ID preserves the license of Joe. A newly synchronized object with a different **Source Anchor** occurs in a duplicated attribute state in Entra ID. Changes for Joe in on-premises Active Directory won't be applied to Joe’s original user (existing object) in Entra ID.
...
1. Check the **Diagnose status** column. The status shows if there's a possible way to fix a sync error directly from Entra ID. In other words, a troubleshooting flow exists that can narrow down the error case and potentially fix it.
...
- Plan, design, and implement Entra Directory Connect (AADC), including password hash synchronization (PHS), pass-through authentication (PTA), seamless single-sign-on (Seamless SSO), and federation
- Manage Entra Directory Connect (AADC)
...
- Troubleshoot synchronization errors
- Implement and manage Entra Directory Connect Health

### Resources

- [What is hybrid identity with Entra ID](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/whatis-hybrid-identity)
...
- [Entra Connect - Configure AD DS Connector Account permission](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-configure-ad-ds-connector-account)
