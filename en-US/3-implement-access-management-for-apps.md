...
- **Register apps using Entra ID** (9 units)
...
**CASB** - Cloud Access Security Broker - An on-premises or cloud-based security policy enforcement point, placed between cloud service consumers and cloud service providers. It combines and interjects enterprise security policies as the cloud-based resources are accessed.
...
Many organizations use AD FS to provide SSO to cloud applications. Moving your AD FS applications to Entra ID for authentication provides significant benefits, especially in terms of cost management, risk management, productivity, compliance, and governance. But understanding which applications are compatible with Entra ID and identifying specific migration steps can be time consuming.
...
The AD FS application activity report in the Azure portal enables you to quickly identify which applications you can migrate to Entra ID. It assesses all AD FS applications for compatibility with Entra ID, checks for any issues, and gives guidance on preparing individual applications for migration. With the AD FS application activity report, you can discover AD FS applications and scope your migration. The AD FS application activity report lists all AD FS applications in your organization that have had an active user logged in within the last 30 days. The activity data is available to users who are assigned any of these admin roles: global reader / administrator, report reader, security reader, application administrator, or cloud application administrator.

### Types of apps to migrate

Migrating all your application authentication to Entra ID is optimal, as it gives you a single control plane for identity and access management.

There are two types of applications to migrate:

1. SaaS applications, which are procured by the organization.
2. Line-of-business applications, which are developed by the organization and not meant to be used by other companies. Your applications might use modern or legacy protocols for authentication. Most SaaS applications use modern authentication protocols and provide guidance on how to enable SSO. Consider first migrating applications that use modern authentication protocols (such as SAML and Open ID Connect). These apps can be reconfigured to authenticate with Entra ID via either a built-in connector in our App Gallery, or by registering the application in Entra ID. Integrate apps using older protocols by using [Application Proxy](https://learn.microsoft.com/en-us/azure/active-directory/manage-apps/what-is-application-proxy) and/or Entra Domain Services.

### Discover AD FS applications that can be migrated

The AD FS application activity report is available in the Azure portal under Entra **Usage and insights** reporting. The AD FS application activity report analyzes each AD FS application to determine whether it can be migrated as-is or after review.
...
2. Select **Entra ID**, and then select **Enterprise applications**.
...
- **Ready to migrate** means the AD FS application configuration is fully supported in Entra ID and can be migrated as-is.
- **Needs review** means some of the application's settings can be migrated to Entra ID, but you'll need to review the settings that can't be migrated as-is.
- **Additional steps required** means Entra ID doesn't support some of the application's settings, so the application can’t be migrated in its current state.
...
This exercise, and the others in this learning path, need an Azure subscription. Use one you already have, or sign up for an Azure trial account. If you already have your own Azure subscription, you skip this task and continue to the next.
...
### Add an app to your Entra tenant
...
5. In the **Browse Entra Gallery** page, in the **Search application** box, enter **GitHub**.
...
This unit describes how to use permissions granted by custom roles in Entra ID to address your application management needs. In Entra ID, you can delegate application creation and management permissions by:

- Restricting who can create applications and manage the applications they create.
- Assigning one or more owners to an application. Assigning owners is a simple way to grant someone the ability to manage all aspects of Entra ID configuration for a specific application.
- Assigning a built-in administrative role that grants access to manage configuration in Entra ID for all applications. Built-in roles are the recommended way to give IT experts broad application configuration permissions, without granting access to the parts of Entra ID unrelated to application configuration.
- Creating a custom role that defines specific permissions, then assigning it to someone either at the scope of a single application, as a limited owner, or at the directory scope covering all applications, as a limited administrator.
...
Assigning owners is a simple way to grant the ability to manage all aspects of Entra ID configuration for a specific application registration or enterprise application. As a system behavior, when a user creates a new application registration, they're automatically added as the first owner. Ownership permissions give the user the ability to manage all aspects of an application registration or enterprise application that they own. The original owner can be removed and additional owners can be added.
...
1. Sign in to your Entra organization with an account that's eligible for the Application Administrator or Cloud Application Administrator for the organization.
...
Entra ID has a set of built-in admin roles for granting access to manage configuration in Entra ID for all applications. These roles are the recommended way to give IT experts broad application configuration permissions, without granting access to the parts of Entra ID unrelated to application configuration.
...
This separation enables you to create a single role definition and then assign it many times at different *scopes*. A custom role can be assigned at organization-wide scope or at the scope of a single Entra object. An example of an object scope is a single app registration. With different scopes, however, an administrator can assign the same role definition to one person over all the app registrations in the organization, and to another person over only a single app or specific app registrations.

Tips when creating and using custom roles for delegating application management:

- Custom roles only grant access in the most current app registration screen of the Entra admin center. They don't grant access in the legacy app registrations screen.
- Custom roles don't grant access to the Entra ID portal when the **Restrict access to Entra ID administration** portal user setting is set to Yes.
...
As you know, Entra ID has a gallery that contains thousands of pre-integrated applications. Many of the applications your organization uses are probably already in the gallery. Once an app is added to your Entra tenant, you can configure its properties, manage user access to it, and configure SSO so users can sign in with their Entra credentials. This unit will show you how to configure properties for the app.

### Configure application (app) properties

To edit the app properties:

1. In the Entra admin center Identity menu, select **Enterprise applications**. Then find and select the application you want to configure.
2. In the **Manage** section, select **Properties** to open the **Properties** pane for editing.
3. Take a moment to understand the options available. The options available will depend on how the app is integrated with Entra ID. For example, an app that uses SAML-based SSO will have fields such as *User access URL* whereas an app that uses OIDC-based SSO won't. Apps added through **Entra ID - App registrations** are by default OIDC-based apps, while apps added **through Entra ID - Enterprise applications** might use any SSO standard. All apps will have fields for configuring when an app appears and can be used. These fields are:
  - **Enabled for users to sign in?** determines whether users assigned to the application can sign in.
  - **User assignment required?** determines whether users who aren't assigned to the application can sign in.
  - **Visible to users?** determines whether users assigned to an app see it in [My Apps](https://myapps.microsoft.com/) and the Microsoft 365 app launcher. (See the waffle menu in the upper-left corner of a Microsoft 365 website.)
...
3. In the Entra admin center, select **Enterprise applications**. Then find and select the application you want to configure.
...
1. In the Entra admin center, select **Enterprise applications**. Then find and select the application you want to configure.
2. In the **Manage** section, select **Properties** to open the **Properties** pane for editing.
...
Besides the existing investigation of OAuth apps connected to your environment, you can set permission policies that notify you automatically when an OAuth app meets certain criteria. For example, you can automatically be alerted when there are apps that require a high permission level and were authorized by more than 50 users. OAuth app policies enable you to investigate which permissions each app requested and which users authorized them for Office 365, and other OAuth apps. You're also able to mark these permissions as approved or banned. Marking them as banned will disable the correlating Enterprise Application.
...
- For example, an admin can set a policy that revokes uncommon apps asking for high permissions, but only when the user who authorized the permissions belongs to the Administrators group.
...
- [What is single-sign-on in Entra ID?](https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/what-is-single-sign-on)
...
In this module, you learn how to implement token customizations and implement and configure consent settings. You also learn how to integrate on-premises apps by using Entra application proxy, and also integrate custom software as a service (SaaS) apps for single-sign-on (SSO). Finally, you explore how to implement application user provisioning and monitor and audit access to Entra ID integrated enterprise applications.
...
Application developers can use optional claims in their Entra ID applications to specify which claims they want in tokens sent to their application.
...
While optional claims are supported in both v1.0 and v2.0 format tokens, and SAML tokens, they provide most of their value when moving from v1.0 to v2.0. One of the goals of the Microsoft identity platform is smaller token sizes to ensure optimal performance by clients. As a result, several claims formerly included in the access and ID tokens are gone from v2.0 tokens: they must now be asked for specifically, per application.

## Implement and configure consent settings

Integrate your applications with the Microsoft identity platform so users can sign in with their work or school account and access the organization's data, which lets you deliver rich data-driven experiences.
...
Malicious applications may attempt to trick users into granting them access to your organization's data. To reduce that risk, it is recommended that you allow user consent only for applications published by a [verified publisher](https://learn.microsoft.com/en-us/azure/active-directory/develop/publisher-verification-overview).
...
By choosing which app consent policies apply for all users, you set the limits: when end users are allowed to grant consent to apps, and when they must instead request administrator review and approval.

- **Disable user consent** – Users can't grant permissions to applications. Users can still sign into apps they previously consented to, or that administrators consented to on their behalf. They can no longer consent to new permissions or to new apps on their own. Only users who have been granted a directory role that includes the permission to grant consent will be able to consent to new apps.
- **Users can consent to apps from [verified publisher](https://learn.microsoft.com/en-us/azure/active-directory/develop/publisher-verification-overview)s or your organization, but only for permissions you choose**– All users can consent only to apps published by a verified publisher and apps registered in your tenant. Users can only consent to the permissions you have classified as `low impact`. You must [classify permissions](https://learn.microsoft.com/en-us/azure/active-directory/manage-apps/configure-permission-classifications) to choose which permissions users are allowed to consent to.
...
- **Custom app consent policy** – For even more control over the conditions governing when users consent, you can [create custom app consent policies](https://learn.microsoft.com/en-us/azure/active-directory/manage-apps/manage-app-consent-policies) and configure them to apply to user consent.
...
## Integrate on-premises apps with Entra application proxy

**What is Application Proxy?** Application Proxy is a feature of Entra ID that enables users to access on-premises web applications from a remote client. Application Proxy includes both the Application Proxy service that runs in the cloud, and the Application Proxy connector that runs on an on-premises server. Entra ID, the Application Proxy service, and the Application Proxy connector work together to securely pass the user sign-on token from Entra ID to the web application.

The Application Proxy for Entra ID provides secure remote access to on-premises web applications. After a single sign-on to Entra ID, users can access both cloud and on-premises applications through an external URL or an internal application portal. For example, Application Proxy can provide remote access and single sign-on to Remote Desktop, SharePoint, Teams, Tableau, Qlik, and line of business (LOB) applications.
...
The following diagram shows how Entra ID and Application Proxy work together to provide single sign-on to on-premises applications.

![Diagram of the Entra Application Proxy process flow. A successful configuration is shown.](https://learn.microsoft.com../../wwl-sci/implement-monitor-integration-of-enterprise-apps-for-sso/media/azure-app-proxxy.png)

1. After the user has accessed the application through an endpoint, the user is directed to the Entra sign-in page.
2. After a successful sign-in, Entra ID sends a token to the user's client device.
...
### Add an on-premises application for remote access through Application Proxy in Entra ID

Launch and interact with this Interactive Guide to learn more about enabling integrated windows authentication to on-premises applications with Entra Application Proxy - **[Enable Integrated Windows Authentication Interactive Guide](https://mslearn.cloudguides.com/guides/Provide%20secure%20remote%20access%20to%20on-premises%20applications%20with%20Azure%20AD%20Application%20Proxy)**


## Integrate custom SaaS apps for single sign-on

![Diagram of Entra ID being the single-sign-on provider for cloud apps. User and external users log into Entra ID, then connect to cloud applications.](https://learn.microsoft.com../../wwl-sci/implement-monitor-integration-of-enterprise-apps-for-sso/media/app-single-sign-on.png)

- You can use Entra ID as your identity system for just about any app. Many apps are already pre-configured and can be set up with minimal effort. These pre-configured apps are published in the Entra ID App Gallery.
- You can manually configure most apps for single-sign-on if they aren't already in the gallery. Entra ID provides several SSO options. SAML-based SSO and OIDC-based SSO.

Effectively, apps can delegate maintenance of their own username and password information to a centralized identity provider, Entra ID as an example. Delegating authentication and authorization enables scenarios such as Conditional Access policies that require a user to be in a specific location or require multifactor authentication. Single-sign-on (SSO) lets a user sign in once, then be signed in automatically to every web app that shares the same centralized directory.

Microsoft identity platform simplifies authorization and authentication for application developers by providing identity as a service. It supports industry-standard protocols such as OAuth 2.0 and OpenID Connect, and offers open-source libraries for different platforms to help you start coding quickly. It allows developers to build applications that sign in all Microsoft identities, get tokens to call Microsoft Graph, other Microsoft APIs, or APIs that developers have built.
...
- **OpenID Connect vs. SAML**: Both OpenID Connect and SAML are used to authenticate a user and are used to enable single-sign-on. SAML authentication is commonly used with identity providers such as Active Directory Federation Services (ADFS) federated to Entra ID and is therefore frequently used in enterprise applications. OpenID Connect is commonly used for apps that are purely in the cloud, such as mobile apps, web sites, and web APIs.

To integrate an application with Entra ID and provide the single-sign-on experience for your users, please see the article ClaimsXRay in Entra ID with Directory Extension, linked below:

[ClaimsXRay in Entra ID with Directory Extension](https://techcommunity.microsoft.com/t5/core-infrastructure-and-security/claimsxray-in-azuread-with-directory-extension/ba-p/1505737)


## Implement application-based user provisioning

In Entra ID, the term app provisioning refers to automatically creating user identities and roles in the cloud ([SaaS](https://azure.microsoft.com/overview/what-is-saas/)) applications that users need access to. In addition to creating user identities, automatic provisioning includes the maintenance and removal of user identities as status or roles change. A common scenario is provisioning an Entra user into applications like [Dropbox](https://learn.microsoft.com/en-us/azure/active-directory/saas-apps/dropboxforbusiness-provisioning-tutorial), [Salesforce](https://learn.microsoft.com/en-us/azure/active-directory/saas-apps/salesforce-provisioning-tutorial), [ServiceNow](https://learn.microsoft.com/en-us/azure/active-directory/saas-apps/servicenow-provisioning-tutorial), and more.
...
Applications in the Entra ID gallery support either manual or automatic provisioning.

- Manual provisioning means there's no automatic Entra provisioning connector for the app yet. User accounts must be created manually. Examples of this include adding users directly into the administrative portal of the app or uploading a spreadsheet with user account details. Consult the documentation provided by the app or contact the app developer to determine what mechanisms are available.
- Automatic means that an Entra provisioning connector has been developed for this application. Follow the setup tutorial for setting up provisioning for the application.

In the Entra ID gallery, applications that support automatic provisioning are designated by a **Provisioning** icon.
...
### Build a System for Cross-domain Identity Management endpoint and configure user provisioning with Entra ID

As an application developer, you can use the System for Cross-Domain Identity Management (SCIM) user management API to provision users and groups automatically between your application and Entra ID. The SCIM specification provides a common user schema for provisioning. When used in conjunction with federation standards like SAML or OpenID Connect, SCIM gives administrators an end-to-end, standards-based solution for access management.

SCIM is a standardized definition of two endpoints: a /Users endpoint and a /Groups endpoint. It uses common Representational state transfer (REST) verbs to create, update, and delete objects, plus a pre-defined schema for common attributes like group name, username, first name, last name, and email. Apps that offer a SCIM 2.0 REST API can reduce or eliminate the pain of working with a proprietary user management API. For example, any compliant SCIM client knows how to make an HTTP POST of a JSON object to the /Users endpoint to create a new user entry. Instead of needing a slightly different API for the same basic actions, apps that conform to the SCIM standard can instantly take advantage of preexisting clients, tools, and code.

![Diagram of Entra ID with user provisioning sharing data with external apps.](https://learn.microsoft.com../../wwl-sci/implement-monitor-integration-of-enterprise-apps-for-sso/media/system-for-cross-domain-identity-management-provision-overview.png)

The standard user object schema and REST APIs for management defined in SCIM 2.0 allow identity providers and apps to integrate with each other more easily. Application developers that build a SCIM endpoint can integrate with any SCIM-compliant client without custom work. Rather than starting from scratch and building the implementation completely on your own, you can rely on a number of open source SCIM libraries published by the SCIM community.


## Monitor and audit access to Entra integrated enterprise applications

With Entra ID reports, you can get the information needed to determine how your environment is doing. With the usage and insights report, you can get an application-centric view of your sign-in data and find answers to the following questions:
...
The Entra audit logs provide records of system activities for compliance. Users in the Security Administrator, Security Reader, Report Reader, Global Reader or Administrator roles can access their data. To access the audit report, select **Audit logs** in the **Monitoring** section of **Entra ID**.
...
Your users can use the My Apps portal to view and start the cloud-based applications they have access to. By default, all the applications a user can access are listed together on a single page. To better organize this page for your users, if you have an Entra ID Premium P1, or P2 license you can set up collections. With a collection, you can group together applications that are related (for example, by job role, task, or project). Then they display on a separate tab for easy usage. A collection essentially filters the applications a user can already access: the user sees only the applications in the collection that are assigned to them.
...
1. Open the Entra admin center and sign in as an admin.
...
You can also use the [My Apps](https://myapps.microsoft.com) portal (`https://myapps.microsoft.com`) to add app collections. My Apps is a web-based portal that is used for managing and launching applications in Entra ID. To work with applications in My Apps, use an organizational account in Entra ID and obtain access granted by the Entra administrator. My Apps is separate from the Azure portal and doesn't require users to have an Azure subscription or Microsoft 365 subscription.
...
- Implement application user provisioning
- Create and manage application collections
- Monitor and audit access/sign-on to Entra ID integrated enterprise applications
...
This unit looks at why applications integrate with Entra ID. Add applications to Entra ID to apply one or more of the services it provides, including:
...
- Directory schema extension attributes: Extend the schema of service principal and user objects to store extra data in Entra ID.

There are two representations of applications in Entra ID: [application objects](https://learn.microsoft.com/en-us/entra/identity-platform/app-objects-and-service-principals) and service principals. The next two sections explain each, and how they interact with one another in the Azure portal.

### What are application objects and where do they come from?

You can manage application objects in the Azure portal through the App Registrations experience. Application objects define and describe the application to Entra ID, enabling your identity provider to know how to issue tokens to the application based on its settings. The application object will only exist in its home directory, even if it's a multitenant application supporting service principals in other directories. The application object includes any of the following (as well as additional information not mentioned here):
...
- Creating a new application using Visual Studio and configuring it to use Entra authentication.
...
You can manage service principals in the Azure portal through the Enterprise Applications experience. Service principals govern an application connecting to Entra ID and can be considered the instance of the application in your directory. Any given application can have at most one application object, registered in a "home" directory. It can also have one or more service principal objects, which represent instances of the application in every directory where it acts.
...
- When users sign in to a third-party application integrated with Entra ID.
...
- When users sign in to Microsoft online services like Microsoft 365.
  - When you subscribe to Microsoft 365 or begin a trial, one or more service principals are created in the directory. They represent the various services used to deliver all of the functionality associated with Microsoft 365.
...
- Add an application to use the Entra Application Proxy.
...
An application has one application object in its home directory. One or more service principals reference it, one in each directory where the application operates, including its home directory.
...
### Who has permission to add applications to my Entra instance?

You can assign roles like Application Administrator, and Cloud Application Administrator to perform these tasks. You **must remember** that, by default, all users in your directory have rights to register the application objects they're developing. They also have discretion over which applications they share / give access to their organizational data through consent. When the first user in your directory connects to an application, and grants consent that will create a service principal. Otherwise, the consent grant information will be stored on the existing service principal.
...
- API owners who use Entra ID for OAuth decide exactly what permissions users are able to grant to applications and which permissions require an admin to agree to. Only admins can consent to larger scopes and more significant permissions, while user consent is scoped to the users' own data and capabilities.
...
Two settings let you turn off those capabilities, if you still want to prevent users in your directory from registering applications and from signing in to applications without administrator approval:
...
- In the Azure portal, go to the User settings section under Entra ID.
- Change **Users can register applications** to **No**.

### Tenancy in Entra ID

Entra organizes objects like users and apps into groups called *tenants*. Tenants enable an administrator to set policies on the users within the organization and the apps that the organization owns to meet their security and operational policies.
...
| Accounts in any Entra directory | Multitenant | All users and guests with a work or school account from Microsoft can use your application or API. This includes schools and businesses that use Microsoft 365. *Use this option if your target audience is business or educational customers.* |
| Accounts in any Entra directory and personal Microsoft accounts (such as Skype, Xbox, Outlook.com) | Multitenant | All users with a work, school, or personal Microsoft account can use your application or API. It includes schools and businesses that use Microsoft 365, as well as personal accounts that are used to sign in to services like Xbox and Skype. *Use this option to target the widest set of Microsoft accounts.* |
...
Each application you want the Microsoft identity platform to perform identity and access management (IAM) for must be registered. Register an app in the Azure portal so the Microsoft identity platform can provide authentication and authorization services for your application and its users. Registering an application establishes a trust relationship between it and the identity provider, the Microsoft identity platform. This holds for a client application, like a web or mobile app, and for a web API that backs a client app.
...
3. Select **Entra ID**, then **App registrations**, and then select your API's app registration.
...
Next, add another example scope named Employees.Write.All that only admins can consent to. Scopes that require admin consent typically provide access to higher-privileged operations, often for client applications that run as backend services or daemons that don't sign in a user interactively.
...
If you successfully added both example scopes described in the previous sections, they appear in the **Expose an API** pane of your web API's app registration, similar to this image:
...
- The app is instantiated with a security principal in Entra ID
...
When the app defines these types of permissions, the resource has fine-grained control over its data and how API functionality is exposed. A third-party app can request these permissions from users and administrators who must approve the request before the app can access data or act on a user's behalf. An app development organization can put the functions of resources into smaller permission sets. Developers can then build third-party apps that request only the specific permissions they need to perform their function. Users and administrators can know exactly what data the app has access to, and they can be more confident that it isn't behaving with malicious intent. Developers should always abide by the concept of least privilege, asking for only the permissions they need for their applications to function.
...
- **Delegated permissions** are used by apps that have a signed-in user present. For these apps, either the user or an administrator consents to the permissions the app requests. The app is then delegated permission to act as the signed-in user when making calls to the target resource. Some delegated permissions can be consented to by non-administrative users, but some higher-privileged permissions require administrator consent. To learn which administrator roles can consent to delegated permissions, see Administrator role permissions in Entra ID.
...
After the user enters their credentials, the Microsoft identity platform endpoint checks for a matching record of user consent. The Microsoft identity platform endpoint asks the user to grant the requested permissions if the user hasn't consented to any of them in the past, and no administrator has consented to them on behalf of the entire organization.

Note

At this time, two permissions are automatically included in the initial consent to an application: offline_access ("Maintain access to data you have given it access to") and user.read ("Sign you in and read your profile"). These permissions are generally required for proper app functionality. offline_access gives the app access to refresh tokens, which are critical for native and web apps. user.read gives access to the sub claim, which lets the client or app correctly identify the user over time and access rudimentary user information.
...
For applications your organization has developed, or that are registered directly in your Entra tenant, you can grant tenant-wide admin consent from App registrations in the Azure portal.
...
1. In a previous exercise, you created an app named Demo app. If necessary, in Microsoft Azure, browse to **Entra ID** then **App registrations** then Demo app.
...
Although a consumer user might grant an application access to this kind of data, organizational users are restricted from granting access to the same set of sensitive company data. If your application requests one of these permissions from an organizational user, that user receives an error message saying they're not authorized to consent to your app's permissions.
...
At this point, Entra ID requires a tenant administrator to sign in to complete the request. The administrator is asked to approve all the permissions that you've requested in the scope parameter.


## Implement application authorization

**Application roles** are used to assign permissions to users. You define app roles by using the Azure portal. When a user signs into the application, Entra ID emits a roles claim for each role granted to the user, both individually and through their group membership.
...
The app governance add-on to Defender for Cloud Apps is a security and policy management capability for OAuth-enabled apps that access Microsoft 365 data through Microsoft Graph APIs. App governance delivers full visibility, remediation, and governance into how these apps and their users access, use, and share your sensitive data stored in Microsoft 365. It does so through actionable insights and automated policy alerts and actions.
...
- **Governance**: Create proactive or reactive policies for app and user patterns and behaviors. They protect your users from non-compliant or malicious apps, and limit the access of risky apps to your data.
...
2. Ensure Office 365 Entra ID apps are enabled.
...
Application registration in Entra ID is the process of ensuring that the identity system is aware of what applications are used. This way you can confirm the user has access to the app and that the app has access to any needed resources. You ensure the security and privacy of users, apps, and your data.
...
By registering your application with Entra ID, you can provide an identity configuration for your application that allows it to integrate with the Microsoft identity platform. This registration process enables several key capabilities:
...
- **Secure authentication**: Configure secure authentication methods. Some confidential client applications can hold credentials securely, like web applications with trusted back-end servers. For those, use client secrets, certificates, or modern alternatives like managed identities for enhanced security.
...
The goal of this module is to teach you how to register your application with Entra ID, then how to configure it to integrate with the Microsoft identity platform. Learn how to customize the branding of your application in the sign-in dialog box. Then explore how to request scope permissions, and how to share a secret with the Microsoft identity platform that proves the app's identity. Finally, learn about single-tenant versus multitenant apps, application objects, and service principal objects, and the relationship between them.


## Plan for app registration

Application registration in Entra ID is the process of ensuring that your identity system is aware of what applications are used. You can confirm the user has access to the app and that the app has access to any needed resources. App registration ensures the security and privacy of users, apps, and your data.

### Benefits of registering an app

When you register your application with Entra ID, you're providing an identity configuration for your application that allows it to integrate with the Microsoft identity platform. Registering the app also allows you to:
...
| Accounts in any Entra directory | Multitenant | All users and guests with a work or school account from Microsoft can use your application or API. Access includes schools and businesses that use Microsoft 365. |
| Accounts in any Entra directory and personal Microsoft accounts (such as Skype, Xbox, Outlook.com) | Multitenant | All users with a work or school, or personal Microsoft account can use your application or API. It includes schools and businesses that use Microsoft 365 as well as personal accounts that are used to sign in to services like Xbox and Skype. |
...
There are two representations of applications in Entra ID:
...
- Provides infrastructure for implementing app provisioning within the app developer's tenant, and to any other Entra tenant.
- Handles user consent during token request time and facilitates the dynamic provisioning of apps across tenants.

Consent is the process of a resource owner granting authorization for a client application to access protected resources, under specific permissions, on behalf of the resource owner. Entra enables users and administrators to dynamically grant or deny consent for the app to access resources on their behalf. And ultimately enables administrators to decide what apps are allowed to do and which users can use specific apps, and how the directory resources are accessed.


## Explore application objects and service principals

After completing the app registration, you have a globally unique instance of the app (the application object) that lives within your home tenant or directory. You also have a globally unique ID for your app (the app/client ID). In the Entra admin center, you can then add secrets or certificates and scopes to make your app work, customize your app's branding in the sign-in dialog, and more.

If you register an application in the Entra admin center, an application object and a service principal object are automatically created in your home tenant. If you register/create an application using the Microsoft Graph APIs, creating the service principal object is a separate step.
...
An Entra application is defined by its application object, which resides in the Entra tenant where the application was registered (known as the application's "home" tenant). An application object is used as a template or blueprint to create one or more service principal objects. A service principal is created in every tenant where the application is used. Similar to a class in object-oriented programming, the application object has some static properties that are applied to all the created service principals (or application instances).
...
To access resources that secured by an Entra tenant, the entity that requires access must be represented by a security principal. This requirement is true for both users (user principal) and applications (service principal). The security principal defines the access policy and permissions for the user/application in the Entra tenant. This enables core features such as authentication of the user/application during sign-in, and authorization during resource access. The types of service principal:
...
- **Managed identity** (Recommended for Azure workloads) - The service principal is used to represent a managed identity. Managed identities eliminate the need for developers to manage credentials, reducing security risks, and operational overhead. Managed identities provide an identity for applications to use when connecting to resources that support Entra authentication. A managed identity is the preferred approach for Azure-hosted applications.
...
A service principal must be created in each tenant where the application is used. It establishes an identity for sign-in and/or access to resources being secured by the tenant. A single-tenant application has only one service principal (in its home tenant), created and consented for use during application registration. A multitenant application also has a service principal created in each tenant where a user from that tenant has consented to its use.
...
- However, restoring the application object through the Entra admin center won't restore its corresponding service principal
...
You can find service principals associated with an application object in the Entra admin center by navigating to the application registration overview and selecting **Managed application in local directory**.
...
This unit demonstrates registering an application in Entra ID using a single-page application (SPA). To register a single-page application in the Microsoft identity platform, complete the following steps. The process is straightforward and requires only a few pieces of information.
...
### Create the app registration
  ...
  Steps are based on the Entra admin center:

1. Sign in to the [Entra admin center](https://entra.microsoft.com/) with appropriate permissions (at least Application Developer role).
...
Follow these steps to add a redirect URI for an app that uses MSAL.js 2.0 or later. MSAL.js 2.0+ supports the authorization code flow with Proof Key for Code Exchange (PKCE) and Cross-Origin Resource Sharing (CORS). That provides enhanced security compared to the legacy implicit grant flow.
...
The registration of your single-page application (SPA) is complete. You configured a redirect URI to which the client is redirected, and any security tokens are sent. Configuring your redirect URI with the Single-page application tile in the **Add a platform** pane sets your application registration to support the authorization code flow with PKCE and CORS.
...
**Best Practice**: New app registrations are hidden from users by default. When you're ready for users to see the app on their My Apps page, go to **Enterprise apps** then **Properties**, and set the **Visible to users?** value to **Yes**.


## Configure app authentication

The settings for each application type, including redirect URIs, are configured in **Platform configurations** in the Entra admin center. Some platforms, like **Web** and **Single-page applications**, require you to manually specify a redirect URI. For other platforms, like **mobile and desktop**, you can select from redirect URIs generated for you when you configure their other settings.
...
1. Open the **Entra admin center**, then under **Applications**, select **App registrations**.
...
**Principle of Least Privilege:** Users and administrators know exactly what data the app can access, and administrators can be more confident that the app isn't behaving with malicious intent. Developers should always request **least privilege**, asking for only the permissions they need for their applications to function.
...
- **Alternative to Group Claims**: App roles provide an alternative to using groups for authorization, which helps avoid group overage issues and doesn't require Entra ID P1 licensing
...
After the application admin creates app roles in your app's registration, IT administrators can assign users and groups to these roles. Your app receives a **roles claim** in tokens (ID tokens for applications, access tokens for APIs). It contains all the signed-in user's assigned roles, as shown in the following token sample:
...
**Assignment Requirements:** When a user or group is assigned to the app, one of the defined app roles must be part of the assignment. If your app has elevated roles for the app (such as "admin"), all users and groups would automatically be assigned the admin role, which violates the principle of least privilege.

**Recommended Approach:** Define a base role (such as "user" or "reader"). Users and groups assigned to the app then receive this base user role, which keeps access levels appropriate.
...
App registration in Entra ID is the foundational process of establishing an identity configuration for your application within the Microsoft identity platform. This process ensures secure integration and provides fine-grained control over authentication and authorization.
...
This module emphasized current Entra ID terminology and tools:

- **Entra admin center** as the primary management interface
